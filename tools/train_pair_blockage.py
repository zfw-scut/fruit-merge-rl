...
def parse_args(argv=None):
    ...
    train.add_argument('--batch-size', type=int, default=16_384)
    ...
    train.add_argument('--context-hidden-dim', type=int, default=40)
    train.add_argument('--head-hidden-dim', type=int, default=48)
    train.add_argument(
        '--autocast-bfloat16',
    ...
    train.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help='平衡等级×标签，并让同一事件的全部正帧合计一次权重。',
    )
    ...
    evaluate.add_argument('--batch-size', type=int, default=32_768)
    evaluate.add_argument(
        '--autocast-bfloat16',
        action=argparse.BooleanOptionalAction,
        default=True,
...
def _dataset_paths(dataset_dir):
    ...
    area = dataset_dir / BLOCKAGE_AREA
    manifest_path = area / 'manifest.json'
    if not manifest_path.is_file():
        raise FileNotFoundError(
            f'current-blockage manifest not found: {manifest_path}'
        )
    paths = tuple(sorted(area.glob(f'{BLOCKAGE_TABLE}-*.pt')))
    if not paths:
        raise FileNotFoundError('current-blockage dataset has no shards')
...
def _load_shard(path):
    ...
    if (
        ...
        raise ValueError(f'invalid current-blockage shard: {path}')
    return payload['columns']


def _batch_columns(columns, indices, device):
    names = PairBlockageModel.REQUIRED_COLUMNS + (
        'episode_id', 'step', 'fruit_id_i', 'fruit_id_j',
        'label', 'level', 'event_id',
...
def _iter_split_batches(
        ...
        *,
        shuffle,
        generator):
    selected_paths = _shuffle_paths(paths, generator) if shuffle else paths
...
def build_training_balance(paths):
    ...
    for path in paths:
        ...
        cell_samples.view(-1).add_(torch.bincount(
            levels * 2 + labels, minlength=cell_samples.numel()
        ))
    ...
    if int(positive_events.min().item()) < 0:
        raise ValueError('positive blockage samples require event ids')
    ...
    present_events = torch.nonzero(
        event_counts > 0, as_tuple=False
    ).flatten()
    ...
    return {
        ...
        'summary': {
            ...
            'by_level': {
                str(level): {
                    'negative_samples': int(cell_samples[level, 0]),
                    'positive_samples': int(cell_samples[level, 1]),
                    'positive_events': int(unique_events[level]),
                }
...
def balanced_sample_weights(batch, balance):
    ...
    if positive.any():
        ...
        if int(event_ids.max()) >= balance['event_counts'].numel():
            raise ValueError('blockage event id exceeds balance table')
...
def _event_state_metrics(probabilities, event_ids, offsets, threshold=0.5):
    event_rows = {}
    for probability, event_id, offset in zip(
            probabilities.tolist(), event_ids.tolist(), offsets.tolist()):
        if int(event_id) < 0:
            continue
        event_rows.setdefault(int(event_id), []).append(
            (int(offset), float(probability))
        )
    detected_offsets = []
    frame_coverages = []
    for rows in event_rows.values():
        positive_rows = [
            offset for offset, probability in rows
            if probability >= float(threshold)
        ]
        frame_coverages.append(len(positive_rows) / max(1, len(rows)))
        if positive_rows:
            detected_offsets.append(min(positive_rows))
    detected_offsets.sort()
    frame_coverages.sort()
    return {
        'events': len(event_rows),
        'detected_events': len(detected_offsets),
        'event_recall': len(detected_offsets) / max(1, len(event_rows)),
        'median_first_detection_offset_from_onset': (
            detected_offsets[len(detected_offsets) // 2]
            if detected_offsets else None
        ),
        'median_positive_frame_coverage_per_event': (
            frame_coverages[len(frame_coverages) // 2]
            if frame_coverages else None
        ),
    }
...
def evaluate_model(
    ...
    for batch in _iter_split_batches(
        ...
        offset_parts.append(batch['offset_from_onset'].long().cpu())
    if sample_count == 0:
    ...
    result = {
        ...
        **risk_metrics(logits, labels),
        'event_state': _event_state_metrics(
            probabilities, event_ids, offsets
        ),
        'by_level': {},
    ...
    for level in range(7, 12):
        ...
        level_result = risk_metrics(logits[mask], labels[mask])
...
def _model_config_from_dataset(args, dataset_dir):
    ...
    return PairBlockageModelConfig(
...
def train(args):
    ...
    balance = (
        ...
        if args.balanced_training else None
    )
    ...
    training_meta = {
        ...
        'seed': args.seed,
        'balanced_training': bool(args.balanced_training),
        'training_balance': balance['summary'] if balance else None,
        'autocast_bfloat16': bool(args.autocast_bfloat16),
    }
    telemetry = TaskTelemetryPublisher(
        task_id=make_task_id('pair-blockage-training', output_dir),
        task_type='supervised_training',
        name='当前几何堵塞检测器训练',
        output_dir=output_dir,
        identity={
            'model': 'Geometry Pair-conditioned Deep Sets',
            'device': str(device),
            'dataset': Path(args.dataset_dir).name,
            'semantics': 'blocked_now',
        },
        metric_schema=[
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for batch in _iter_split_batches(
                paths, SPLIT_TRAIN, args.batch_size, device,
            ...
            with _autocast(device, args.autocast_bfloat16):
                ...
                weights = (
                    ...
                    if balance else torch.ones_like(losses)
                )
                loss = (losses * weights).sum() / float(args.batch_size)
            loss.backward()
            ...
            weight_sum_total += float(weights.sum().item())
            samples += int(logits.numel())
        validation = evaluate_model(
            model, paths, SPLIT_VALIDATION, args.batch_size * 2, device,
            autocast_bfloat16=args.autocast_bfloat16,
        )
        row = {
            'epoch': epoch,
            'train_loss': loss_sum / max(1e-6, weight_sum_total),
            'train_samples': samples,
            'train_weight_mass': weight_sum_total,
            'epoch_seconds': time.perf_counter() - epoch_started,
            'validation': validation,
        }
        history.append(row)
        score = float(validation.get('macro_average_precision', math.nan))
        ...
        telemetry.update(
            phase='训练', current=epoch, total=int(args.epochs), unit='epoch',
            metrics={
    ...
    test_result = evaluate_model(
        model, paths, SPLIT_TEST, args.batch_size * 2, device,
        autocast_bfloat16=args.autocast_bfloat16,
    )
    final = {
        'status': 'complete',
    ...
    telemetry.complete(
        ...
        metrics={
            ...
            'test_macro_average_precision': test_result.get(
...
def evaluate_checkpoint(args):
    ...
    result = evaluate_model(
        model, paths, split, args.batch_size, device,
        autocast_bfloat16=args.autocast_bfloat16,
    )
