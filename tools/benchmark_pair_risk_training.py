...
from daxigua.rl.pair_risk import PairRiskModel, SPLIT_TRAIN  # noqa: E402
from tools.train_pair_risk import (  # noqa: E402
    _autocast,
    _dataset_paths,
    _iter_split_batches,
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description='短跑标定轻量堵塞风险网络的端到端训练吞吐。'
    )
...
def benchmark_candidate(args, paths, balance, device, batch_size):
    ...
    _run_steps(
        ...
        args.warmup_steps,
        device,
        args.autocast_bfloat16,
    )
    _synchronize(device)
    if device.type == 'cuda':
        torch.cuda.reset_peak_memory_stats(device)
    started = time.perf_counter()
    samples, last_loss, _iterator = _run_steps(
        model,
        optimizer,
        iterator,
        iterator_factory,
...
def benchmark(args):
    ...
    if not valid:
        raise RuntimeError('all pair-risk benchmark candidates failed')
    ...
    result = {
        'format_version': 1,
        'purpose': 'pair_risk_training_throughput_calibration',
        ...
        'model_parameters': sum(
            parameter.numel()
            for parameter in PairRiskModel(
                _model_config_from_dataset(args, args.dataset_dir)
            ).parameters()
        ),
