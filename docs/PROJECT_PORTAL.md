...
- **实时训练**：原生展示云端/本地训练进度、训练队列、GPU资源、损失、评估曲线、
  Merge Potential大规模统计以及合成步距预测工作流进度；
- **场景实验室**：原生编辑实时物理场景，对照21动作模型预测与真实执行结果，并按需
  显示完整圆障碍加权 Voronoi / Free-Space Graph及逐水果合成步距颜色预测。
...
合成步距预测工作流同样通过既有8765只读API同步，不另开端口。遥测服务只扫描规范目录
中的小型清单与逐epoch `metrics.jsonl`：场景采集显示对局数、吞吐和完整场景数；离线标签
显示已处理分片和有效监督槽位；预测器训练显示epoch、当前/最佳验证NLL、区间准确率及
训练曲线。训练过程每10秒原子刷新心跳，门户不读取场景张量，也不向任务发送控制指令。
...
- 场景实验室的Q模型与合成步距预测器使用独立checkpoint参数和设备白名单；步距参数留空
  时只在规范`runs/merge_distance`目录中自动发现最近成品；
