...
仍是 V1 历史证据，但不能批准 V2 的 250k。V2 的 5k、10k 和 250k 均须使用空目录
从 update 0 开始，禁止恢复旧 checkpoint 或复用旧 hot/cold/causal replay。设计、
无未来泄漏证明和兼容表见
[`STRUCTURE_AWARE_GNN_V2.md`](STRUCTURE_AWARE_GNN_V2.md)。

不要在 5k 和 10k 门禁完成前启动 250k。当前进度和证据以
`docs/training_runs/FIRST_250K_READINESS.md` 为准。

本次第一次正式长训有一项已经明确批准的单次例外：2026-07-28，用户批准使用同一
结构感知 V2 方案、从 update 0 独立完成且全部 required checks 通过的 10k 校准，
替代没有单独执行的 V2 5k 顺序证据。10k 覆盖了 5k 的稳定性观察窗口，且没有恢复
旧 checkpoint；因此不再为了补齐形式顺序重复跑 5k，也不要求额外 25k。该例外不
放宽最终提交上的全量测试、正式 250k preflight、全新空目录、资源监控或恢复门禁。
...
普通 ZIP/源码归档没有 `.git`，不能作为正式 5k、10k 或 250k 的项目根目录；否则
...
正式 250k 必须记录明确的 commit，并保持工作树干净。然后执行：
...
preflight 不启动持续训练，但会验证正式 250k 解析后的结构参数、CUDA 前后向、六维
...
conda run --no-capture-output -n python-torch \
  ...
  --output runs/preflight/first_250k_pre_smoke.json \
  --snapshot-audits 32 \
  --min-free-gb 80 \
  --min-memory-gb 64 \
...
- `formal_training_config_contract` 通过，确认输入确实是冻结的 250k/CUDA/H256/L4/
...
preflight 通过只表示静态和小规模物理链路闭合，不等于 250k 已经获准启动。
...
- `env_steps_per_second`、内存、显存和 checkpoint 体积是否可承受 250k。
...
`dqn_causal_fast30_h128_l3_n3_250k`：
...
必须写入 readiness 记录，并在启动 250k 前重新冻结配置和 preflight。
...
- 吞吐、内存和 checkpoint 保存时间可以外推到 250k；
- 所有暂停阈值均已排除或有书面解释。
...
`missing_optional`。5k、10k 或 250k 阶段结束后同步最小完整分析包时使用完整模式：
...
5. 更新 `docs/training_runs/FIRST_250K_READINESS.md`；
6. 5k/10k 结果通过后再进入下一阶段，250k 结果另建正式训练总结。
