...
构图只读取动作前稳定边界的 `GameState`、21 个当前候选投放位置以及当时已经显示给
...
当前默认场景为 `560x1120 / spawn_y=252`，规范动作列由 15 增加到 21。动作节点
共享同一套 GNN 参数，dueling Q 读出不含固定长度的动作输出层，因此尺寸和动作数
变化不会改变 V2 模型参数 shape。同架构 checkpoint 可以用 `--init-checkpoint`
做 weights-only 初始化：仅加载 online 权重并复制到新 target。

这不是 resume。新 run 必须重新创建 optimizer、TD/causal replay、RNG、epsilon、
update/env 计数和评估状态；旧 15 动作图、动作索引和归因 mask 不得混入 21 动作
replay。训练入口会以 `strict=True` 校验模型结构，并将继承/重置清单写入
`initialization.json`。
