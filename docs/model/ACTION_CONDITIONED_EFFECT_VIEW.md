...
- 状态：基线动作探针、辅助动作效果监督和结构化首次接触头已实现并进入正式训练；FiLM 和高层计划仍未实现
- 适用范围：低层 FiLM-GNN-Dueling Double DQN 的动作效果编码
- 非目标：本页不规定层数、隐藏维度、稀疏算子或具体训练参数

动作结果预测没有改变历史第一版 GNN-DQN 基线；它已经作为可开关的独立升级项接入。
历史基线仍保持单策略头和无辅助损失，新配置及完整标签语义见
`AUXILIARY_ACTION_EFFECT_LEARNING.md`。当前已有完整管线效果证据，但单模块因果仍需消融。
...
当前物理边还显式编码一组“配对最低结果圆”几何。圆心位于两颗水果的圆心连线上，
按两者标准半径比例偏向较小水果；同级水果时自然退化为模拟器使用的圆心中点。结果圆
半径取两者较高等级再升一级后的合成物理半径，表示这对关系最终完成合成时能够产生的
最低结果尺度。若较高等级已经是 L11，则结果圆不存在，并由独立有效标志与零半径表示。
该几何只描述潜在结果的位置和尺度，不表达合成概率、所需时间或人工好坏判断。
...
当前升级配置复用 21 个动作探针的 `action_hidden`，并行预测合成、q0 谱系、首次接触、
全局前三个新水果、q0 最终后代状态、稳定与风险变化。训练时只 gather 实际执行动作的
预测计算辅助损失；未执行动作不使用真实结果监督。

策略读出可配置多个结构相同的 Dueling 头。Replay 为每条 transition 固化 bootstrap
mask，按头训练 Double DQN，并用去除 value 偏置后的动作偏好标准差表示模型决策
不确定性。该不确定性可以在低 epsilon 阶段替换原随机探索预算中的一部分动作。
