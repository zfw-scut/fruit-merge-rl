...
- 状态：V2与V2.1首轮RTX 5090训练均已完成；V2.1修复奖励长期偏正，但真实分数只比V2
  提高约1%，仍低于分数基线22%～23%，详见
...
- RTX 5090成对热路径门禁中，V2.1相对`score_v1`吞吐下降0.855%，性能目标达标；
- 131,072条Replay样本中，近似无合成奖励均值由V2的+0.01098变为-0.00201，正奖励率
  由97.43%降至37.22%，固定补偿导致的奖励偏正已经修复；
- 4096局最终评估中，V2.1相对V2仅提高1.17%（30 FPS）和1.39%（120 FPS），相对
  分数基线仍低22.10%和23.31%；
- V2.1最外侧动作占比升至36.91%，但墙边投放属于常见玩法，该比例不单独视为缺陷；
  约26.36%的近似合成transition得到负奖励；
- 参考损失是给定状态下所有实际动作共用的标量，能校正奖励基线，但不能直接改变基础
  21列空间损失对动作的即时排序。当前暂停空间奖励训练，恢复`score_v1`并先验证基础GNN
  传播深度；后续若重启Reward V2研究，不再只叠加与动作无关的补偿。
