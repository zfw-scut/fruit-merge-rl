...
下一轮同时启用`active_learning_shadow_bonuses = [0.5, 1, 2, 4, 8]`。影子计算复用同一次
actor前向的`mean_Q`和策略头分歧，只计算候选动作，不执行、不写Replay、不消耗随机数，
因此训练行为仍由`uncertainty_bonus = 1`决定，18M epsilon实验的行为边界不变。
...
因此总探索率仍由 epsilon 决定，不会另开一个隐藏探索通道。首轮评估发现“进入主动学习
mask”不等于“实际改变贪心动作”，新监控进一步区分有效动作改变率和主动/贪心重叠率。
当前阈值、替换比例和bonus都不是已验证最优参数。

每个影子bonus按统计窗口保存：

- 相对纯`argmax(mean_Q)`的动作改变率与重叠率；
- 只在动作确实改变时的平均Q代价；
- 只在动作确实改变时的平均不确定性增益。

公共量纲同时保存21动作Q范围、Top1/Top2间隔、动作不确定性均值和最大值。这样可以判断
固定bonus是在打破近似平局，还是足以选择明显更不确定但Q值稍低的动作。
...
实时面板新增总/DQN/辅助损失、策略头分歧和主动学习比例，并增加五组辅助损失、评估期
L7～L11新水果生成密度、bonus影子动作改变率和决策量纲曲线。归档
`training_curves.png`扩为4×2。
