# 辅助动作效果学习与策略不确定性

- 状态：结构化首次接触头、128M父训练+12M旁路及120 FPS迁移均已完成；单模块因果仍待消融
- 更新日期：2026-08-12
- 最新归档配置：`configs/gnn_dqn_auxiliary_action_l5_128m.toml`、`configs/gnn_dqn_auxiliary_action_structured_120fps_transfer_16m.toml`
- 对照边界：历史模型与历史配置保持单策略头、无辅助损失，仍可加载

> 正文按实验发生顺序保留“下一轮”等历史措辞，不代表当前推荐；最新效果与决策以
> `../model_evaluations/COMPARISON_MATRIX.md` 为准。

首轮与第二轮正式训练分别验证过6.4M和18M降至0.05的epsilon曲线。第二轮影子诊断表明
固定bonus存在量纲依赖且实际改动作率有限。随后直接在主轨迹中执行排名主动动作的
24M/34M实验表明，高频主动动作会持续破坏正常对局质量。下一轮推荐配置恢复6.4M最低点，
保留无bonus的排名融合，但把主动动作改为不修改父轨迹的单步克隆旁路；该轮正式训练
已经归档为`model-auxiliary-action-single-step-branch-r4`。随后加入的结构化首次接触头只
改变下一轮训练结构，不追写历史checkpoint的能力。历史运行身份和历史报告保持不变。

## 1. 目标与非目标

本轮实现让模型在学习 Q 值的同时，学习“一次实际投放到下一个决策边界之间发生了
什么”。它包含 CUDA 事件采集、GPU Replay 标签、多策略头 bootstrap、辅助预测头、
分组损失、低 epsilon 主动探索和云端可视化。

本轮没有实现：未执行动作的反事实标签、行为段归因、关键决策触发器、世界模型、课程
学习、高层规划或 BMCTS。关键决策事实框架仍作为稀有样本和未来反事实任务的独立出口，
不是本次密集辅助监督的运行依赖。

## 2. 标签语义

辅助监督只对应实际执行动作。新水果事件按一次投放内的全局合成事件顺序取前三个，
不要求位于 q0 谱系；由 q0 碰撞间接触发的合成也因此可进入标签。

| 分组 | 标签 |
| --- | --- |
| 合成 | 是否合成、合成次数、`score_delta`、水果总数变化 |
| q0 谱系 | q0 是否参与、谱系深度、最终后代等级 |
| 首次接触 | 地面/左墙/右墙/水果多标签、主接触类型、具体接触水果槽位、位置及几何残差、接触水果等级差、法向量、时间、法向接近速度 |
| 新水果 | 前三次新水果是否存在、位置、等级；按全局事件顺序，不限 q0 谱系 |
| 决策边界 | q0 最终后代是否存在及 `x/y/vx/vy/角速度` |
| 稳定与风险 | 稳定、等待超时、终止、模拟时长、危险进度变化、决策边界是否越线 |

“首次接触类型”允许同一最早物理帧内多个类型同时为真；另保存一个按法向接近速度选择
的主接触，供位置、等级差和法向量使用。无接触、非水果接触、新水果不存在、q0 最终
消失均通过显式有效掩码区分，不用全零值冒充有效标签。

结构化候选编号固定为`0=未接触、1=地面、2=左墙、3=右墙、4=动态水果、
5+slot=投放前场内水果`。
`slot`是投放前定长状态中的水果槽位，只属于监督事实，不加入策略状态。CPU与CUDA
模拟器都在首次接触发生时直接记录槽位，避免事后按位置重新匹配导致合成或移动后歧义。
若接触槽位在投放前未激活，标签会落到“动态水果”候选，覆盖未稳定局面中途新生成水果
这一稀有情况，不会强行冒充某个投放前水果。

## 3. 热路径

```text
actor 一次前向
→ 多策略头均值 Q + 模型分歧
→ 父轨迹只执行 epsilon 随机 / 贪心，完整保留24M正常transition
→ 按累计预算抽取少量决策状态，GPU内复制到旁路模拟器
→ 每个父状态分别执行4个排名主动动作，每个克隆只推进一次投放
→ 旁路结果写入独立GPU Replay，不修改也不续跑父环境
→ Replay.begin_append：复制决策前状态并固化 bootstrap mask
→ CUDA simulator.step：固定大小事件累计
→ 动作效果标签压缩
→ Replay.finish_append：状态、TD 字段、bootstrap mask、辅助标签全部留在 GPU
→ learner：完整父batch + 小型旁路batch，一次联合前向和一次优化器更新
```

事件累计没有逐帧 Python 对象、CPU 回传或同步归档。CUDA 每个环境只维护固定大小的
首次接触与 q0 谱系摘要；已有定长合成事件数组用于提取全局前三个新水果。

## 4. 多策略头与 bootstrap

共享 GNN、队列编码和动作探针后接多个表述相同 21 动作决策的 Dueling 策略头。默认
推荐 5 个头。前向兼容输出是各头 Q 值均值，详细输出保留 `[B, H, 21]`。

每条 transition 写入 Replay 时只生成一次 Bernoulli bootstrap mask，并保证至少一个头
有效；以后反复采样该 transition 都使用同一 mask。Double DQN 的动作选择、Target 估值
和 Huber loss 按头计算，再只在该头有效样本上聚合。这样分歧来自不同训练子集，而不是
每次 update 临时随机屏蔽造成的噪声。

动作不确定性使用去除每个头动作均值后的优势偏好标准差：

```text
centered_q[h, a] = q[h, a] - mean_a(q[h, a])
uncertainty[a] = std_h(centered_q[h, a])
```

它减少不同 value 偏置对决策分歧的污染。

## 5. 主动学习动作与轨迹隔离

旧的直接排名主动模式与epsilon随机探索是两个独立、互斥的分支；该模式仍保留为历史
配置兼容能力，但下一轮正式配置已设置`active_learning_enabled = false`，不再让主动动作
进入父轨迹。历史直接模式的概率定义为：

```text
p_active = 0                              , epsilon >= 0.50
p_active = 0.40 * (0.50-epsilon)/0.45    , 0.05 < epsilon < 0.50
p_active = 0.40                           , epsilon <= 0.05

if u < p_active:              排名主动学习
elif u < p_active + epsilon:  epsilon均匀随机
else:                         argmax(mean_Q)
```

旁路模式复用相同的双排名：对21个动作分别计算`mean_Q`降序名次和策略头分歧降序名次，
以“名次和、两者较差名次、价值名次、动作下标”的顺序得到完整候选列表。每个被抽中的
父状态先排除父轨迹实际动作，再取前4个替代动作，各复制一个环境并分别执行一次投放。
这里没有Q与不确定性的加权相加，也没有量纲变换参数。

正式24M配置额外生成4M旁路transition，即抽取1M个父决策状态；按全部24M父决策计约
4.17%。预算从6.4M父transition开始按累计目标均匀释放，单个父决策批次内不重复抽同一
状态；抽样使用独立且可恢复的设备RNG，不额外消耗父动作选择的随机数序列。旁路模拟器
的下一决策状态只用于本次标签和Replay，写入后即被下一批克隆覆盖，
不会形成低质量的长期主动对局。

## 6. 辅助头和 loss

辅助头读取共享动作效果表示，但 learner 先按实际动作 gather，再计算监督。其他 20 个未
执行动作不会接收真实结果标签。损失按五组等权聚合后乘总权重：

- `aux_loss_merge`；
- `aux_loss_q0_lineage`；
- `aux_loss_first_contact`；
- `aux_loss_generation`；
- `aux_loss_outcome`。

分类使用交叉熵或二元交叉熵，连续量使用 Smooth L1，并按接触、新水果、最终后代等有效
掩码聚合。总训练损失为：

```text
loss = bootstrap_double_dqn_loss
     + auxiliary_loss_weight * mean(auxiliary_group_losses)
```

首次接触子任务在`structured_contact_enabled=true`时不再直接把整幅场景压成一个绝对
坐标。共享动作表示对69个定长候选打分：5个特殊候选加64个水果槽位；水果候选同时读取
对应GNN节点表示和投放线几何特征。候选确定后，模型为每个候选预测二维位置残差，最终
位置是“解析几何先验+该候选残差”。训练仍只gather实际执行动作，并进一步按真实候选
gather残差；候选交叉熵和残差Smooth L1替换旧的主类型交叉熵和绝对位置Smooth L1，
因此首次接触分组仍维持7项等权，不扩大该分组在总辅助损失中的权重。

这仍是一次共享编码、一次前向传播。它没有采用顺序事件链，也不会因前三次新水果而
执行三遍模型；额外成本只来自固定候选打分和小型二维残差读出。

旁路Replay与父Replay分别采样并分别求均值，正式配置每次更新使用256条父样本和64条
旁路样本。联合损失为：

```text
loss = parent_loss + 0.25 * branch_loss
```

因此父batch没有被旁路样本替换；旁路样本占本次实际读取样本的20%，且在两路loss尺度
相近时，其归一化有效梯度权重也是`0.25 / 1.25 = 20%`。这两个20%是显式配置语义，
不是效果已经最优的结论。

## 7. 配置与兼容性

历史兼容默认值是：

```toml
[model]
policy_head_count = 1
action_effect_enabled = false
structured_contact_enabled = false

[dqn]
bootstrap_probability = 0.8
auxiliary_loss_weight = 0.2
active_learning_enabled = false
```

因此旧 TOML 和旧 checkpoint 的模型参数名保持兼容。启用新配置后会增加策略头、辅助头
和 Replay 标签，属于新的模型结构，不能把它误写成与旧 checkpoint 完全相同的续训。

`structured_contact_enabled`默认关闭，所以`feb10ec`、`8235ef9`和`bdf04ae`等历史辅助
checkpoint仍按原状态字典严格加载；它们返回旧的主类型与绝对位置预测，结构化候选字段
为空。当前推荐TOML已打开该开关，必须从新模型开始训练，不能用`bdf04ae`无损续训成
结构化头模型。

首轮正式24M配置实际使用5层GNN、5个策略头、`score_v1`、batch 256、1536个环境和
5组辅助监督。正式运行前的CUDA编译、短smoke、显存和吞吐门禁使配置从初稿收敛到该档；
多头和辅助Replay确实提高了learner成本。

下一轮旁路配置继续保留父batch 256，新增64条旁路learner batch、256环境旁路模拟器和
262144容量的独立Replay。本地CUDA预检按每条记录实测投影，正式旁路Replay约占
1.04GB显存，两套Replay合计约5.22GB。checkpoint保存模型、优化器、父/旁路累计预算与
Replay元数据，不保存完整Replay；恢复时父Replay按既有流程重建，旁路Replay从恢复后的
新旁路样本重建，并在达到一个旁路learner batch后恢复联合更新。

## 8. 监控与产物

实时面板保留总/DQN/辅助损失、策略头分歧，并增加父/旁路预算、独立Replay占用、旁路
吞吐、旁路DQN/辅助损失和联合采样比例。主动学习诊断只保留“实际执行的旁路动作价值
名次与不确定性名次的窗口相关系数”；不保存逐动作
排名、Q代价、分歧增益或多组影子候选。相关系数为正表示两种名次同向，负值表示当前
Top-K选择更多体现价值与不确定性的权衡；样本不足或方差为零时记为空值。

面板旁路进程每2秒采样GPU利用率和显存，保存轻量时间序列；实时面板与4×2归档曲线均
展示GPU/显存占用率。训练正常完成后写入`run_status.json`，原训练入口自动在同一端口
切换为最终只读面板，明确显示“已正常完成”并保持可访问，直到用户停止该进程。可用
`--exit-after-completion`显式关闭这一行为。切换前会释放trainer对象和CUDA缓存，面板
存活不等于训练仍在运行，也不会继续占用训练期显存。
高等级密度按“每千次投放生成数”归一化，避免不同评估局长和局数直接比较原始计数。

接口预览图使用确定性合成数据，只用于检查字段、图例和布局，不是模型效果证据：

- `docs/assets/auxiliary_action_learning/training_curves_preview.png`；
- `docs/assets/auxiliary_action_learning/dashboard_panels_preview.png`。

## 9. 验证与结论边界

已验证事实：CPU 标签链路、全局前三个新水果、bootstrap mask、多策略头均值、实际动作
辅助 loss、父状态GPU内复制、父轨迹不变、每克隆单步推进、独立旁路Replay、父/旁路
联合更新、旁路进度恢复、CUDA 扩展编译、CUDA首次接触与q0谱系，以及正式配置的CUDA
预检和训练入口smoke均通过。

首轮正式训练的完整配置在同4096评估seed上，相对5层Fast基线的30/120 FPS均分分别
提高13.62%/14.86%，高等级水果密度与L11消除率也提高；详见
`../model_evaluations/model-auxiliary-action-r1.md`。这证明完整配置有效，不证明收益来自辅助
任务、策略多头或主动学习中的某一个，也不证明分歧已校准、5个头和当前loss权重最优。
单步旁路方案已经完成正式24M父轨迹+4M旁路训练，其策略评估与原因边界见
`../model_evaluations/model-auxiliary-action-single-step-branch-r4.md`。结构化首次接触头
尚未正式训练，因此本页对其只能记录接口、监督和回归事实，不能写成预测精度或策略效果
已经提升。

## 10. 场景实验室中的单场景验证

带辅助动作头的 checkpoint 可通过 `tools/open_scenario_lab.py --serve --checkpoint ...`
载入场景实验室。启用实时预测后，稳定场景或队列、场上水果变化会自动触发模型评估；
单次评估复用共享动作编码，一次返回21个动作的Q值和辅助效果预测，切换A0～A20不会重复
前向。前端可先显示预测的首次接触、前三次全局新水果和q0最终后代，
再通过真实21动作物理评估显示同语义事实和位置误差。

基础画布、动作预测图层、误差线、法向量、概率明细和场景空间评估位于独立齿轮设置页；
空间评估默认关闭并与动作预测分层。主页面只保留刷新预测、执行验证、动作切换及
“预测+真实/仅预测/仅真实”。该工具用于定位具体失败样本，训练仍只监督实际执行
动作；场景实验室不会给其他20个动作生成伪标签，也不进入Replay或训练热路径。
