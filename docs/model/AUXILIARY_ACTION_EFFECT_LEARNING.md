...
载入场景实验室。单次模型评估复用共享动作编码，一次返回21个动作的Q值和辅助效果预测；
切换A0～A20不会重复前向。前端可先显示预测的首次接触、前三次全局新水果和q0最终后代，
再通过真实21动作物理评估显示同语义事实和位置误差。

图层、误差线、法向量和概率明细位于独立齿轮设置页；主页面只保留模型评估、执行验证、
动作切换及“预测+真实/仅预测/仅真实”。该工具用于定位具体失败样本，训练仍只监督实际执行
