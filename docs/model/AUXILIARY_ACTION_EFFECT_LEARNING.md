...
推荐首轮 24M 配置使用 5 层 GNN、5 个策略头、`spatial_v2_1`、batch 512 和 5 组辅助
监督。云端部署仍应先跑 CUDA 编译、短 smoke、显存和吞吐门禁，再决定是否维持 1792 个
环境；多头和辅助 Replay 会提高显存及 learner 成本。
...
尚不能得出的结论：辅助任务提高了分数、样本效率或高等级水果频率；分歧已完成校准；
主动学习优于普通随机探索；5 个头和当前 loss 权重最优。这些必须由正式训练和固定种子
30/120 FPS 评估支持，完成后再更新模型报告与比较矩阵。
