...
- 状态：20,000局数据、离线标签、独立训练与固定测试已完成；冻结预测器已接入场景实验室诊断层
...
2026-08-17正式部署使用7,168个并行环境、场景间隔8和回传间隔16。60秒试测中，4,096
环境约为16,527投放/秒，7,168环境约为20,599投放/秒；正式采集目录为
`runs/analysis/sab128_merge_distance_20k_20260817`，后续训练目录为
`runs/merge_distance/sab128_merge_distance_20k_20260817`。这些数据只用于部署与吞吐选择，
正式预测器共123,979参数，`final.pt`的SHA-256为
`8f96212a8eff99ef5e2fd01911512df23a043565c473b9d6c684cf22fcff17e6`。固定测试集包含
1,244,520个已解析水果样本，精确区间准确率为47.23%，相邻区间准确率为64.57%；这些指标
描述区间分类效果，不代表RL策略得分提升。
...
正式数据关联、模型前后向、分片训练、checkpoint保存、严格加载和固定测试均已完成。
冻结`final.pt`现可由场景实验室按水果ID显示最大概率区间、置信度和最终合成概率；该接入
仍是只读诊断工具，不代表加入RL后会提高分数。后续RL融合前仍需单独测量目标训练批量下的
前向吞吐、显存占用及融合消融结果。
