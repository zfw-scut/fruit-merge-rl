...
- `CLOUD_SERVER_LOCAL.md`：云服务器实例连接凭据（SSH 地址 + 密码）的本地明文登记，
  **仅本机保存、gitignored、不进入仓库历史**；Agent 需要调用云服务器或面板转发时先
  读取此文件获取当前实例连接信息，服务器更换后更新该文件并删除已弃用实例；
...
- `codex/65_同步最新模型评估至统一面板_2026_08_11.md`：把128M结构化旁路与120 FPS迁移模型同步到总览和模型图谱，增加双物理身份、L11事件与同谱系增益展示。
- `codex/66_场景实验室统一为训练同源Tensor_CUDA物理_2026_08_12.md`：用训练同源Tensor/CUDA增量Kernel替换实时Pymunk后端，删除旧实现与依赖并建立逐位一致性测试。
