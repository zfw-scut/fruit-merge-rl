...
对战胜/负/平和对战最高分。单人和对战只在正式结束时记录；AI 演示结果不会混入
玩家历史。每局使用稳定 session ID 做幂等门禁，即使从结束瞬间的草稿恢复，也
不会重复累计。计数使用
非负校验和溢出保护。重置历史需要再次确认，只清除历史和防重账本，不改变设置。
...
AI 演示与挑战 AI 会显示带 Q 版头像的动态对话气泡。`READY`、`THINKING`、
`HESITATING`、`HAPPY`、`SURPRISED`、`WORRIED` 分别表达就位、思考、犹豫、
得分开心、对玩家高等级连锁的惊讶和临近危险线。气泡有弹入、轻微漂浮、淡出和
消息优先级，重要反应不会被普通状态瞬间覆盖。
...
前模拟点击。`-Screen` 支持
`home/solo/duel/demo/settings/history/exit/new/result`：
