...
七类语料分别保存在 `assets/dialogue/*.txt`，每类以 1024 条完整句子为基础，
并可继续追加人工选定的原句。运行时不会组合前缀或词块，只从完整句子中用无
放回洗牌袋抽取；重复原句会保留为显式权重，使更常见的口头反应自然提高出现
概率。当前语料共 7597 行：`welcome` 1042、`thinking` 1066、
`hesitating` 1037、`ready` 1132、`happy` 1266、`surprised` 1027、
`worried` 1027。普通发言结束后随机静默约 3.5～7 秒，任何两次开口还有 2.2 秒
硬间隔；思考等低价值状态只按概率发言，高连锁和危险可以突破软冷却但不能刷屏。
同一 `ScoreSequence` 或同一对战回合/阵营的连续合成最多请求一次反馈，危险提示
按进入危险状态的边沿触发。

语料口吻定位为“可爱但不幼稚的真人陪玩”：单句允许从一个非空字符到 24 个
Unicode 字符，
以随口短句、轻微自言自语、语气词和适量拟声词为主；避免“执行、目标、方案、
判断完成”等系统提示腔，也不靠网络梗、连续感叹号或过度撒娇制造热闹。

颜文字和文字使用两条表现通道，但颜文字不再无条件响应每次合成。完整文字气泡
自带表情并计入表情冷却；没有说话时的独立颜文字还要通过自己的全局间隔、同类
冷却和概率门禁。普通合成只会偶尔短促反应，精彩操作和危险事件响应率更高，但
依然不能连续闪烁。因此 AI 不会因为少说话而完全没有反应，也不会在连续合成中
每次都跳出一个表情。被当前气泡挡住的高优先级事件会短暂排队；最高优先级危险
反应可在旧句完整展示至少 0.85 秒后结束旧气泡并兑现一次，避免一次性事件被吞。
所谓“完整语料”是 APK 内预生成静态句库，不需要网络，也不会在手机端调用在线
大模型。
