...
- 当 Box2D 确认本次连锁稳定后，同组分值会一起加速吸入左上角“分数”卡；人工
  持续投放时另有表现等待上限，避免浮分永久滞留。卡片发光、播放奖励音并将显示
  分数快速滚动到已经即时结算的真实分数。动画只属于表现层，不延迟规则计分、
  模型快照或奖励逻辑。
...
顶部的齿轮按钮打开设置页，旁边的记录按钮打开历史页。两页沿用暖色果园卡片和
站酷快乐体；打开浮层时当前游戏暂停推进，返回后从原局面继续。
...
历史页记录总完成局数、历史最高分、单局最多合成大西瓜数、累计合成大西瓜数、
对战胜/负/平和对战最高分。经典局与对战局只在正式结束时各记一次，计数使用
非负校验和溢出保护。重置历史需要在短时间内再次确认，只清除历史，不改变设置。

## AI 对战模式与结算

点击顶部“经典/对战”按钮切换模式；已有局内进度时需要再次确认，避免误触丢局。
对战不是把同一个物理世界轮流交给两方，而是同时维护玩家与 AI 两个完全独立的
Box2D 场景、分数、危险计时和大西瓜计数：

- 两方每轮共享同一颗待投水果和同一条后续队列，不能因随机序列获得优势；
- 每轮倒计时按真实时间推进，不受游戏速度设置影响；物理和危险线时间仍跟随
  游戏速度；
- 玩家可以在本轮内自由拖动并提交；双方都提交后短暂停顿，再共同进入下一颗；
- 倒计时结束仍未提交时，玩家按当前预览位置自动投放；AI 会在时间充足时等待
  自己的场景稳定并调用模型，临近截止时改用安全启发式，最后保证按时投放；
...
仍允许切换前景查看两个场景；倒计时结束后才出现胜利、失败或平局结算页。

经典结算和对战结算都会显示“已超越一定比例玩家”的提示。这个百分比由本地内部
...
双场景规则集中在
`android/core/src/main/java/com/fruitmerge/ai/game/DuelMatch.java`，持久化数据
契约集中在同目录的 `GameProfileStore.java`。前者不负责绘制或 AI 推理，后者不
保存当前局面的临时状态，便于分别做纯规则测试。
...
字体生成器会扫描 Android `core/app` 主源码中的真实 Java 字符串字面量并跳过
注释，新增中文设置或结算文案不会再依赖手工猜测字表。
...
设置、历史和对战画面也能直接进入，不需要在自动截图前模拟点击：
...
- AI 对战是同一设备上的玩家对内置 AI，不是联网匹配或远程 PvP；双方共享水果
  序列，但物理求解仍是两个本地独立世界。
- 设置与历史保存在 Android 应用私有 Preferences 中；卸载应用或在系统设置中
  清除应用数据会删除这些本地记录。
