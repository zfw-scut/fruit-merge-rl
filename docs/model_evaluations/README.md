...
1. [`COMPARISON_MATRIX.md`](COMPARISON_MATRIX.md) 中目标谱系的相关行；
2. 目标谱系最新报告；
3. 只有设计新方案时，再读本文的证据规则和对应设计文档；
4. 只有需要历史依据时，再定点查询经验索引或原始 run。

普通参数、UI、工具和 bug 修复不需要阅读本知识库。
...
| `auxiliary-action-structured-branch-128m-r5` | `score_v1`，128M父轨迹 + 12M隔离单步旁路 + 结构化首次接触头 | `2e0f836` | [`model-auxiliary-action-structured-branch-128m-r5.md`](model-auxiliary-action-structured-branch-128m-r5.md) | **零初速度旧协议**30/120 FPS为7161.42/6551.90；当前V4物理补零复评为6430.51，当前排序见新基线报告 |
