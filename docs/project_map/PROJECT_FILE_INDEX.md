...
| `src/daxigua_rl/attribution/` | 完整状态归因模块。当前提供 worker 内使用的纯只读数据契约和静态状态分析器；后续再接 tracker、Reward V2 和因果 replay。 | `StateAnalyzer`、`StateAnalysis`、`FruitAnalysis`、`FreeSpaceRegionAnalysis` |
...
| `docs/rl/CAUSAL_ATTRIBUTION_V1.md` | 第一次大规模训练的完整状态归因 V1 规格。 | 固定 Reward V2、状态分析、归因事件、因果 Q 排序、反事实预算、测试和长训流程；前四步基础语义、数据契约和 `StateAnalyzer` 已经落地。 |
...
- `StateAnalyzer`：从稳定 `GameState`、15 个动作候选和 `TransitionKey` 生成
  `StateAnalysis`。它只做静态只读近似，不推进物理，也尚未接入 collector、Reward V2
  或 tracker。
