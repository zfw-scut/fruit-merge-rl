...
<div class="app">
  <header class="topbar">
    ...
    <div class="command-group push"><button id="settings-open" class="icon-button" title="显示与预测设置" aria-label="打开设置">⚙</button><button id="evaluate-model-top" class="command">评估模型倾向</button><button id="model-continuous-top" class="command model-continuous">启动持续决策</button><button id="evaluate-top" class="primary">执行并验证 21 个动作</button></div>
  </header>
    ...
    <aside class="panel"><div class="panel-scroll">
      ...
      <div id="tab-result" class="tab-page">
        <section class="section">
          ...
          <div class="run-stack" style="margin-top:9px"><button id="evaluate-model" class="secondary">评估当前场景的模型倾向</button><button id="model-continuous" class="primary model-continuous">启动模型持续决策</button></div>
...
<div id="settings-page" class="settings-page" aria-hidden="true">
  <div class="settings-shell" role="dialog" aria-modal="true" aria-labelledby="settings-title">
    ...
    <div class="settings-content">
      <section class="settings-group"><h3>辅助预测图层</h3><p>控制画布上的模型预测、真实物理事实和误差参考。</p><div class="settings-grid">
...
const DEFAULT_VIEW_SETTINGS={showPrediction:true,showActual:true,showErrorLines:true,showNormals:true,showGenerations:true,showQ0Final:true,showProbabilities:false,autoSelectModelAction:true};
...
function closeSettings(){$('settings-page').classList.remove('open');$('settings-page').setAttribute('aria-hidden','true');saveViewSettings();renderEffectComparison();draw()}
...
function invalidateEvaluation(){evaluation=null;evaluationScene=null;selectedResultAction=null;modelEvaluation=null;modelEvaluationScene=null;$('result-empty').hidden=false;$('result-content').hidden=true;renderModelEvaluation();renderEffectComparison();draw()}
...
function activeEffectPrediction(){const rows=modelEvaluation?.action_effect_predictions;return Array.isArray(rows)?rows[effectAction()]||null:null}
function activeEffectActual(){return evaluation?.actions?.[effectAction()]?.action_effect||null}
function effectLayerVisible(kind){if(kind==='prediction')return effectViewMode!=='actual'&&viewSettings.showPrediction;if(kind==='actual')return effectViewMode!=='prediction'&&viewSettings.showActual;return false}
...
function drawActionEffectOverlay(){if(activeSideTab!=='result')return;const predicted=activeEffectPrediction(),actual=activeEffectActual(),showPrediction=effectLayerVisible('prediction'),showActual=effectLayerVisible('actual');if(!predicted&&!actual)return;
...
function drawSpaceOverlay(){if(activeSideTab!=='result')return;const action=activeResult(),slot=action?.space_slots?.[selectedSpaceSlot];if(!slot)return;const before=slot.before,after=slot.after,xs=after.drop_x||before.drop_x||[];ctx.save();ctx.lineWidth=1;
...
function draw(){ctx.clearRect(0,0,BOARD.width,BOARD.height);const bg=ctx.createLinearGradient(0,0,0,BOARD.height);bg.addColorStop(0,'#17263a');bg.addColorStop(1,'#0e1724');ctx.fillStyle=bg;ctx.fillRect(0,0,BOARD.width,BOARD.height);
  ...
  const invalid=state.showWarnings&&!evaluating?overlaps():new Set;(evaluationFruits||visibleSceneFruits()).forEach(fruit=>drawFruit(fruit,{invalid:invalid.has(fruit.id),selected:!evaluationFruits&&fruit.id===state.selectedId}));
...
function renderLiveStatus(){const autoEditing=Boolean(autoEditPending||transientEditActive||autoEditFinishing),controlled=modelContinuous.running,evaluating=evaluationViewActive(),resultOpen=activeSideTab==='result',phaseLabel=evaluationPhase==='before'?'投放前':'投放后',label=resultOpen?(evaluating?`评估快照 · ${phaseLabel}`:'评估结果 · 等待快照'):!backendConnected?'离线编辑':state.done?'对局已结束':controlled?`模型持续决策 · #${modelContinuous.decision_count||0}`:autoEditing?'按住编辑 · 松手恢复':state.livePaused?'已暂停 · 编辑模式':'实时运行';$('canvas-status').textContent=evaluating?`${label} · A${selectedResultAction} · ${evaluation.physics_fps||state.fps} FPS`:backendConnected?`${label} · 物理 ${state.physicsFps||120} FPS · 显示同步`:label;$('canvas-tip').textContent=evaluating?`固定显示 A${selectedResultAction} 的${phaseLabel}局面；实时世界仍在后台运行`:resultOpen?'点击“评估 21 个动作”生成静态诊断快照':controlled?(modelContinuous.message||'模型将在每次连续稳定后自动选择并投放。'):autoEditing?'拖动水果调整位置；松手后自动同步并恢复实时物理':backendConnected&&!state.livePaused?(state.tool==='select'?'选择模式：按住水果直接编辑，右键水果直接删除':'放置模式：空白处拖动投放；按住已有水果直接编辑；右键删除'):'暂停编辑：左键放置 · 拖动调整 · 右键删除 · 滚轮切级';$('live-state-label').textContent=label;$('live-score').textContent=String(state.score||0);$('live-frame').textContent=String(state.physicsFrame||0);$('live-stable').textContent=state.done?'结束':controlled&&modelContinuous.phase==='inference'?'推理中':state.stable?'稳定':'运动中';const toggle=$('live-toggle');toggle.disabled=!backendConnected||state.done||autoEditing;toggle.textContent=state.livePaused?'恢复实时运行':'暂停并进入编辑';toggle.classList.toggle('running',backendConnected&&!state.livePaused&&!state.done);document.querySelector('.canvas-badge i').style.background=resultOpen?'#83d2ff':!backendConnected?'#f2c14e':controlled?'#63c7a0':autoEditing?'#f2c14e':state.livePaused?'#75b9ee':'#79d7a8'}
...
function renderModelEvaluation(){const button=$('evaluate-model'),topButton=$('evaluate-model-top'),continuousButton=$('model-continuous'),continuousTop=$('model-continuous-top'),root=$('model-action-results'),running=Boolean(modelContinuous.running);button.disabled=!backendConnected||!modelAvailable||modelBusy||running;topButton.disabled=!backendConnected||!modelAvailable||modelBusy||running;button.textContent=modelBusy?'正在推理…':'评估当前场景的模型倾向';topButton.textContent=modelBusy?'模型推理中…':'评估模型倾向';[continuousButton,continuousTop].forEach(control=>{control.disabled=!backendConnected||!modelContinuousAvailable||modelControlBusy;control.classList.toggle('running',running)});continuousButton.textContent=modelControlBusy?'正在切换…':running?'停止模型持续决策':'启动模型持续决策';continuousTop.textContent=modelControlBusy?'正在切换…':running?'停止持续决策':'启动持续决策';if(!modelEvaluation){$('model-state-label').textContent=running?`持续决策中 · #${modelContinuous.decision_count||0}`:modelAvailable?'已加载 · 等待评估':'未加载';$('model-action').textContent='—';$('model-q').textContent='—';$('model-latency').textContent='—';root.replaceChildren();const identity=modelIdentity||{};$('model-note').textContent=running?(modelContinuous.message||'等待第一次模型决策。'):modelAvailable?`${identity.checkpoint||'checkpoint'} · ${identity.checkpoint_sha256||'未知摘要'} · 单次评估不会投放水果。`:'启动服务时通过 --checkpoint 指定模型。';return}const values=modelEvaluation.q_values.map(Number),best=Number(modelEvaluation.action),min=Math.min(...values),max=Math.max(...values),span=Math.max(1e-8,max-min);$('model-state-label').textContent=running?`持续决策中 · #${modelContinuous.decision_count||0}`:'已完成 · 只读推理';$('model-action').textContent=`A${best} · x ${Number(modelEvaluation.drop_x).toFixed(1)}`;$('model-q').textContent=Number(modelEvaluation.selected_q).toFixed(4);$('model-latency').textContent=`${Number(modelEvaluation.inference_ms).toFixed(2)} ms`;root.replaceChildren(...values.map((value,index)=>{const item=document.createElement('div');item.className='action-result'+(index===best?' best':'')+(index===selectedResultAction?' selected':'');item.title=`A${index} · Q ${value.toFixed(6)}`;item.innerHTML=`<i style="height:${8+82*(value-min)/span}%"></i><span>${index}</span>`;item.onclick=()=>{state.probe=index;selectedResultAction=index;renderProbe();renderModelEvaluation();if(evaluation)renderEvaluation();renderEffectComparison();renderLiveStatus();draw()};return item}));const sameScene=evaluation&&modelEvaluationScene&&evaluationScene&&JSON.stringify(modelEvaluationScene)===JSON.stringify(evaluationScene),modelReward=sameScene?evaluation.actions[best]:null,identity=modelEvaluation.model||modelIdentity||{};$('model-note').textContent=running?`${modelContinuous.message||'持续决策运行中'} · 已投放 ${modelContinuous.decision_count||0} 次。`:modelContinuous.error?modelContinuous.error:modelReward?`模型首选 A${best} 的实际空间奖励 ${Number(modelReward.reward).toFixed(5)}；Reward V2.1 最优为 A${evaluation.best_action}。`:`${identity.checkpoint||'checkpoint'} · ${identity.checkpoint_sha256||'未知摘要'} · Q 值仅表示模型相对偏好。`}
async function evaluateModelScene(scene){const response=await fetch('/api/model/evaluate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scene})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);if(!Array.isArray(payload.q_values)||payload.q_values.length!==21)throw new Error('模型结果必须包含21个 Q 值');modelEvaluation=payload;modelEvaluationScene=JSON.parse(JSON.stringify(scene));if(viewSettings.autoSelectModelAction){state.probe=Number(payload.action);selectedResultAction=Number(payload.action)}renderProbe();renderModelEvaluation();renderEffectComparison();draw();return payload}
...
async function requestEvaluation(mode='all'){const requestMode=mode==='settle'?'probe':mode,detail={mode:requestMode,scene:cloneScene()};window.dispatchEvent(new CustomEvent('daxigua:scenario-request',{detail}));selectTab('result');if(!backendConnected){toast('后端未连接','请使用 --serve 启动真实物理与Reward V2.1服务。');return}toast('开始真实评估','正在并行执行21个投放动作…');try{if(modelAvailable)await evaluateModelScene(detail.scene);const response=await fetch('/api/evaluate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(detail)}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);applyEvaluation(payload,detail.scene);toast('评估完成',`已返回 ${payload.actions.length} 个真实物理结果，并对照模型倾向`)}catch(error){toast('评估失败',String(error.message||error))}}
function applyEvaluation(payload,sourceScene=null){if(!payload||!Array.isArray(payload.actions)||payload.actions.length!==21)throw new Error('评估结果必须包含21个 actions');evaluation=payload;evaluationScene=JSON.parse(JSON.stringify(sourceScene||cloneScene()));evaluationPhase='after';const sameModelScene=modelEvaluation&&modelEvaluationScene&&JSON.stringify(modelEvaluationScene)===JSON.stringify(evaluationScene);selectedResultAction=sameModelScene?Number(modelEvaluation.action):(Number.isInteger(payload.selected_action)?payload.selected_action:(Number.isInteger(payload.best_action)?payload.best_action:0));state.probe=selectedResultAction;selectedSpaceSlot=0;$('result-empty').hidden=true;$('result-content').hidden=false;renderEvaluation();selectTab('result')}
...
document.querySelectorAll('[data-view-setting]').forEach(input=>input.onchange=()=>{viewSettings[input.dataset.viewSetting]=input.checked;saveViewSettings();renderEffectComparison();draw()});
$('settings-open').onclick=openSettings;$('settings-close').onclick=closeSettings;$('settings-done').onclick=closeSettings;$('settings-reset').onclick=()=>{viewSettings={...DEFAULT_VIEW_SETTINGS};syncSettingsControls();saveViewSettings();renderEffectComparison();draw()};$('settings-page').onclick=event=>{if(event.target===$('settings-page'))closeSettings()};
