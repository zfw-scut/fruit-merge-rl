...
let settleRequestId=0,settling=false;
...
function place(point){if(state.fruits.length>=64){toast('无法继续放置','场景最多包含 64 个水果');return}const fruit={id:nextId++,level:state.level,x:point.x,y:point.y,physicsRadius:spec(state.level).dropped_physics_radius};clampFruit(fruit);state.fruits.push(fruit);state.selectedId=fruit.id;remember();renderUi();draw();requestNaturalSettle()}
...
async function requestNaturalSettle(options={}){if(!backendConnected)return;const fast=options.fast!==false,requestId=++settleRequestId,source=physicsSnapshot(),selectedId=state.selectedId;settling=true;renderUi();try{const response=await fetch('/api/settle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scene:cloneScene(),fast})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);if(requestId!==settleRequestId||source!==physicsSnapshot())return;state.fruits=(payload.fruits||[]).map(item=>({id:Number(item.id),level:Number(item.level),x:Number(item.x),y:Number(item.y),physicsRadius:Number(item.physics_radius)}));nextId=Math.max(1,...state.fruits.map(item=>item.id+1));state.selectedId=state.fruits.some(item=>item.id===selectedId)?selectedId:null;invalidateEvaluation();if(history.length)history[history.length-1]=snapshot();future=[];if(payload.merge_count)toast('场景已自然落稳',`发生 ${payload.merge_count} 次合成`);else if(payload.done)toast('场景已自然落稳','当前局面触发终局');else if(payload.settle_timeout)toast('物理仍在运动','已到达单次等待上限，可继续编辑或再次整理');}catch(error){if(requestId===settleRequestId)toast('自然落稳失败',String(error.message||error))}finally{if(requestId===settleRequestId){settling=false;renderUi();draw()}}}
...
canvas.addEventListener('pointerup',()=>{if(drag){const changed=drag.before!==snapshot();if(changed)remember();drag=null;renderUi();draw();if(changed)requestNaturalSettle()}});canvas.addEventListener('pointerleave',()=>{hover=null;$('coordinate').style.display='none';if(!drag)draw()});
