"""自定义场景实验室的无依赖前端页面与Reward V2.1诊断视图。"""
...
.evaluation-view-switch{display:grid;grid-template-columns:1fr 1fr;padding:3px;border-radius:9px;background:#edf0f5}
.evaluation-view-switch button{height:34px;border:0;border-radius:7px;background:transparent;color:var(--muted);font-size:11px;cursor:pointer}
.evaluation-view-switch button.active{background:#fff;color:var(--text);box-shadow:0 1px 4px rgba(30,42,62,.12)}
...
.model-action-results{margin-top:9px}.model-action-results .action-result i{background:linear-gradient(#a9c7e4,#527da8)}
.model-action-results .action-result.best i{background:linear-gradient(#63c7a0,#107c5b)}
.model-continuous.running{color:#fff;border-color:#075a9e;background:var(--accent)}
...
.effect-toolbar{display:grid;grid-template-columns:repeat(3,1fr);gap:4px;padding:3px;border-radius:9px;background:#edf0f5}.effect-toolbar button{height:31px;border:0;border-radius:7px;background:transparent;color:var(--muted);font-size:11px;cursor:pointer}.effect-toolbar button.active{background:#fff;color:var(--text);box-shadow:0 1px 4px rgba(30,42,62,.12)}
.effect-summary{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:9px}.effect-card{padding:9px;border:1px solid var(--stroke);border-radius:9px;background:#fff;min-width:0}.effect-card span{display:block;color:var(--muted);font-size:9px;margin-bottom:4px}.effect-card strong{display:block;font-size:11px;line-height:1.4;overflow-wrap:anywhere}.effect-detail{margin-top:8px;padding:9px;border-radius:8px;background:#f5f7fa;color:var(--muted);font:10px/1.55 ui-monospace,monospace;white-space:pre-wrap}
.settings-page{position:fixed;inset:0;z-index:100;display:none;background:rgba(20,29,42,.34);backdrop-filter:blur(8px)}.settings-page.open{display:grid;place-items:center}.settings-shell{width:min(720px,calc(100vw - 40px));max-height:calc(100vh - 48px);display:flex;flex-direction:column;border:1px solid rgba(255,255,255,.75);border-radius:16px;background:#f8f9fb;box-shadow:0 24px 70px rgba(18,28,43,.28);overflow:hidden}.settings-header{height:58px;display:flex;align-items:center;gap:12px;padding:0 18px;border-bottom:1px solid var(--stroke);background:rgba(255,255,255,.82)}.settings-header h2{margin:0;font-size:16px}.settings-header span{color:var(--muted);font-size:11px}.settings-close{margin-left:auto;width:34px;height:34px;border:1px solid var(--stroke);border-radius:9px;background:#fff;cursor:pointer}.settings-content{overflow:auto;padding:18px;display:grid;gap:14px}.settings-group{padding:15px;border:1px solid var(--stroke);border-radius:12px;background:#fff}.settings-group h3{margin:0 0 4px;font-size:13px}.settings-group p{margin:0 0 13px;color:var(--muted);font-size:10px;line-height:1.5}.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px 22px}.settings-footer{padding:12px 18px;border-top:1px solid var(--stroke);display:flex;justify-content:flex-end;gap:8px;background:#fff}
...
<div class="app">
  <header class="topbar">
    <div class="brand"><div class="brand-mark" aria-hidden="true">◇</div><div class="brand-copy"><strong>场景实验室</strong><span>Scenario Lab · Reward V2.1</span></div></div>
    ...
    <div class="command-group push"><button id="settings-open" class="icon-button" title="显示与预测设置" aria-label="打开设置">⚙</button><button id="evaluate-model-top" class="command">刷新模型预测</button><button id="model-continuous-top" class="command model-continuous">启动持续决策</button><button id="evaluate-top" class="primary">执行并验证动作预测</button></div>
  </header>
      ...
      <div id="tab-scene" class="tab-page active">
        ...
        <section class="section">
          ...
          <div class="run-stack">
            ...
            <button id="settle" class="secondary">验证当前探针动作</button>
            <button id="evaluate" class="primary">执行并验证 21 个动作</button>
          </div>
          <div class="backend-note"><i></i><span>启用实时预测后，稳定场景会自动更新；执行验证只读取当前快照，不会改动实时世界。</span></div>
        </section>
      </div>

      <div id="tab-result" class="tab-page">
        <section class="section">
          <div class="section-title"><strong>模型倾向</strong><span id="model-state-label" class="hint">未加载</span></div>
          <div class="live-strip">
            <div><span>首选动作</span><strong id="model-action">—</strong></div>
            <div><span>首选 Q 值</span><strong id="model-q">—</strong></div>
            <div><span>推理耗时</span><strong id="model-latency">—</strong></div>
          </div>
          <div id="model-action-results" class="action-results model-action-results"></div>
          <div class="run-stack" style="margin-top:9px"><button id="evaluate-model" class="secondary">刷新当前场景预测</button><button id="model-continuous" class="primary model-continuous">启动模型持续决策</button></div>
          <div class="backend-note"><i></i><span id="model-note">加载 checkpoint 后，可在这里查看模型对 21 个投放坐标的 Q 值；该操作不会投放水果。</span></div>
        </section>
        <section class="section">
          <div class="section-title"><strong>辅助动作预测</strong><span id="effect-state-label" class="hint">等待模型预测</span></div>
          <div class="effect-toolbar"><button class="active" data-effect-view="both">预测 + 真实</button><button data-effect-view="prediction">仅预测</button><button data-effect-view="actual">仅真实</button></div>
          <div id="effect-summary" class="effect-summary"></div>
          <div id="effect-detail" class="effect-detail">加载带辅助动作头的 checkpoint 后，可以先查看预测位置，再执行真实物理验证。</div>
        </section>
        <div id="result-empty" class="empty-result"><div class="empty-illustration">⌁</div><strong>等待第一次真实验证</strong><p>模型预测可以实时显示；执行验证后，这里会加入真实首次接触、生成水果和q0后代结果。</p></div>
        <div id="result-content" hidden>
          <section id="evaluation-snapshot-section" class="section">
            <div class="section-title"><strong>评估快照画面</strong><span id="evaluation-view-label" class="hint">—</span></div>
            <div class="evaluation-view-switch"><button data-evaluation-phase="before">投放前</button><button class="active" data-evaluation-phase="after">投放后</button></div>
            <div class="alignment-note">结果画面固定在发起评估时的状态；实时物理仍在后台继续，不会覆盖当前诊断。</div>
          </section>
          <section id="space-summary-section" class="section" hidden>
            ...
            <div class="result-summary">
              ...
              <div class="result-card"><span>最佳无合成参考损失</span><strong id="space-compensation">—</strong></div>
          ...
          <section id="space-slots-section" class="section" hidden><div class="section-title"><strong>未来水果空间</strong><span class="hint">21列近似</span></div><div id="space-slots" class="space-slots"></div><div class="alignment-note">投放前 q1～q3 与投放后 q0～q2 严格对齐；新随机 q3 不参与当前奖励。画布中绿色为新增空间、红色为损失空间，虚线和实线分别表示投放前后可抵达深度。</div></section>
          <section id="space-actions-section" class="section" hidden><div class="section-title"><strong>21 动作空间奖励</strong><span class="hint">点击切换结果</span></div><div id="action-results" class="action-results"></div><div id="result-message" class="result-message"></div></section>
        </div>
      </div>
    </div></aside>
  </main>
...
<div id="settings-page" class="settings-page" aria-hidden="true">
  <div class="settings-shell" role="dialog" aria-modal="true" aria-labelledby="settings-title">
    <div class="settings-header"><div><h2 id="settings-title">场景实验室设置</h2><span>低频显示选项集中在这里，主页面只保留评估与动作切换。</span></div><button id="settings-close" class="settings-close" aria-label="关闭设置">×</button></div>
    <div class="settings-content">
      <section class="settings-group"><h3>基础画布</h3><p>这些选项只改变画布显示，不改变场景状态。</p><div class="settings-grid">
        <label class="switch-row"><span>显示 21 个动作锚点</span><span class="switch"><input data-view-setting="showAnchors" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示空间参考网格</span><span class="switch"><input data-view-setting="showGrid" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>高亮几何异常</span><span class="switch"><input data-view-setting="showWarnings" type="checkbox"><span></span></span></label>
      </div></section>
      <section class="settings-group"><h3>辅助动作预测</h3><p>实时模式在场景变化并达到可决策状态时更新；一次推理覆盖全部21个动作。</p><div class="settings-grid">
        <label class="switch-row"><span>实时自动更新预测</span><span class="switch"><input data-view-setting="realtimePrediction" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示模型预测</span><span class="switch"><input data-view-setting="showPrediction" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示真实物理结果</span><span class="switch"><input data-view-setting="showActual" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>连接预测与真实误差</span><span class="switch"><input data-view-setting="showErrorLines" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示首次接触法向量</span><span class="switch"><input data-view-setting="showNormals" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示前三次新水果</span><span class="switch"><input data-view-setting="showGenerations" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>显示 q0 最终后代</span><span class="switch"><input data-view-setting="showQ0Final" type="checkbox"><span></span></span></label>
      </div></section>
      <section class="settings-group"><h3>场景空间评估</h3><p>旧的Reward V2.1可达空间模块独立控制，默认关闭，不与动作效果预测混排。</p><div class="settings-grid">
        <label class="switch-row"><span>显示空间评估模块</span><span class="switch"><input data-view-setting="showSpaceEvaluation" type="checkbox"><span></span></span></label>
      </div></section>
      <section class="settings-group"><h3>信息密度</h3><p>概率明细只影响右侧说明，不改变模型推理和真实模拟。</p><div class="settings-grid">
        <label class="switch-row"><span>显示类别概率明细</span><span class="switch"><input data-view-setting="showProbabilities" type="checkbox"><span></span></span></label>
        <label class="switch-row"><span>模型评估后自动切到首选动作</span><span class="switch"><input data-view-setting="autoSelectModelAction" type="checkbox"><span></span></span></label>
      </div></section>
    </div>
    <div class="settings-footer"><button id="settings-reset" class="secondary" style="padding:0 14px">恢复默认</button><button id="settings-done" class="primary">完成</button></div>
  </div>
</div>
...
let state={fruits:[],queue:[1,2,3,4],selectedId:null,level:1,tool:'place',fps:120,probe:10,
  title:'空白场景',livePaused:false,score:0,stepCount:0,physicsFrame:0,
  stable:true,done:false,liveSequence:-1};
let nextId=1,hover=null,drag=null,history=[],future=[],historyLock=false;
let evaluation=null,evaluationScene=null,evaluationPhase='after',activeSideTab='scene';
let selectedResultAction=null,selectedSpaceSlot=0,backendConnected=false,liveEventSource=null;
let modelAvailable=false,modelIdentity=null,modelEvaluation=null,modelEvaluationScene=null,modelBusy=false;
let modelContinuousAvailable=false,modelContinuous={running:false,phase:'idle',decision_count:0},modelControlBusy=false;
const DEFAULT_VIEW_SETTINGS={showAnchors:true,showGrid:true,showWarnings:true,realtimePrediction:true,showPrediction:true,showActual:true,showErrorLines:true,showNormals:true,showGenerations:true,showQ0Final:true,showSpaceEvaluation:false,showProbabilities:false,autoSelectModelAction:true};
let viewSettings=loadViewSettings(),effectViewMode='both';
let realtimePredictionTimer=null,realtimePredictionInFlight=false,realtimePredictionQueued=false,realtimePredictionLastKey='',currentPredictionSceneKey='',modelEvaluationKey='',modelRequestSerial=0;
...
function loadViewSettings(){try{return {...DEFAULT_VIEW_SETTINGS,...JSON.parse(localStorage.getItem('daxigua.scenarioLab.viewSettings')||'{}')}}catch(_error){return {...DEFAULT_VIEW_SETTINGS}}}
function saveViewSettings(){try{localStorage.setItem('daxigua.scenarioLab.viewSettings',JSON.stringify(viewSettings))}catch(_error){/* 本地文件预览可能禁用存储。 */}}
function syncSettingsControls(){document.querySelectorAll('[data-view-setting]').forEach(input=>{input.checked=Boolean(viewSettings[input.dataset.viewSetting])})}
function openSettings(){syncSettingsControls();$('settings-page').classList.add('open');$('settings-page').setAttribute('aria-hidden','false')}
function applyVisualizationVisibility(){['space-summary-section','space-slots-section','space-actions-section'].forEach(id=>$(id).hidden=!viewSettings.showSpaceEvaluation)}
function closeSettings(){$('settings-page').classList.remove('open');$('settings-page').setAttribute('aria-hidden','true');saveViewSettings();applyVisualizationVisibility();renderEffectComparison();scheduleRealtimePrediction({immediate:true});draw()}
function predictionSceneKey(scene){return JSON.stringify({fps:scene.fps,queue:scene.queue,fruits:scene.fruits.map(({id,level,x,y,vx,vy,physics_radius})=>[id,level,x,y,vx,vy,physics_radius])})}
function modelPredictionMatches(scene){return Boolean(modelEvaluation&&modelEvaluationKey===predictionSceneKey(scene))}
function realtimePredictionEnabled(){return Boolean(viewSettings.realtimePrediction&&viewSettings.showPrediction&&backendConnected&&modelAvailable&&modelIdentity?.action_effect_available&&!modelContinuous.running&&!modelBusy)}
function scheduleRealtimePrediction({immediate=false,live=false}={}){if(!realtimePredictionEnabled())return;if(live&&!state.stable)return;if(live){if(realtimePredictionTimer!==null)return;realtimePredictionTimer=setTimeout(()=>{realtimePredictionTimer=null;runRealtimePrediction()},350);return}clearTimeout(realtimePredictionTimer);realtimePredictionTimer=setTimeout(()=>{realtimePredictionTimer=null;runRealtimePrediction()},immediate?0:120)}
async function runRealtimePrediction(){if(!realtimePredictionEnabled())return;if(realtimePredictionInFlight){realtimePredictionQueued=true;return}const scene=cloneScene(),key=predictionSceneKey(scene);if(key===realtimePredictionLastKey&&modelEvaluation)return;realtimePredictionInFlight=true;realtimePredictionQueued=false;renderModelEvaluation();renderLiveStatus();try{const payload=await evaluateModelScene(scene,{selectModelAction:false,quiet:true});if(payload)realtimePredictionLastKey=key}catch(error){$('effect-state-label').textContent='实时预测暂不可用';console.error(error)}finally{realtimePredictionInFlight=false;renderModelEvaluation();renderLiveStatus();draw();if(realtimePredictionQueued)scheduleRealtimePrediction({immediate:true})}}
...
function renderLiveFrame(now){liveRenderHandle=requestAnimationFrame(renderLiveFrame);if(!backendConnected||state.livePaused||state.done||!visualFramesReady||activeSideTab==='result')return;visualFruits=interpolateLiveFruits(now);draw()}
...
function invalidateEvaluation(){evaluation=null;evaluationScene=null;selectedResultAction=null;modelEvaluation=null;modelEvaluationScene=null;modelEvaluationKey='';$('result-empty').hidden=false;$('result-content').hidden=true;renderModelEvaluation();renderEffectComparison();draw()}
function remember(){if(historyLock)return;invalidateEvaluation();const value=snapshot();currentPredictionSceneKey=predictionSceneKey(JSON.parse(value));if(history.at(-1)!==value){history.push(value);if(history.length>80)history.shift()}future=[];updateUndo();scheduleRealtimePrediction()}
function updateUndo(){const editable=(!backendConnected||state.livePaused)&&activeSideTab!=='result'&&!modelContinuous.running&&!autoEditPending&&!transientEditActive&&!autoEditFinishing;$('undo').disabled=!editable||history.length<2;$('redo').disabled=!editable||!future.length}
...
function evaluationViewActive(){return activeSideTab==='result'&&Boolean(evaluation&&evaluationScene&&activeResult())}
function normalizeEvaluationFruit(item){const level=Number(item.level);return {id:Number(item.id),level,x:Number(item.x),y:Number(item.y),vx:Number(item.vx)||0,vy:Number(item.vy)||0,angle:Number(item.angle)||0,angularVelocity:Number(item.angular_velocity)||0,ageFrames:Number(item.age_frames)||0,physicsRadius:Number(item.physics_radius)||spec(level).merged_physics_radius}}
function evaluationViewFruits(){if(!evaluationViewActive())return null;if(evaluationPhase==='before')return evaluationScene.fruits.map(normalizeEvaluationFruit);return (activeResult().result_fruits||[]).map(normalizeEvaluationFruit)}
function effectAction(){return selectedResultAction===null?Number(state.probe):Number(selectedResultAction)}
function activeEffectPrediction(){if(activeSideTab!=='result'&&(!currentPredictionSceneKey||modelEvaluationKey!==currentPredictionSceneKey||backendConnected&&!state.livePaused&&!state.stable))return null;const rows=modelEvaluation?.action_effect_predictions;return Array.isArray(rows)?rows[effectAction()]||null:null}
function activeEffectActual(){return evaluation?.actions?.[effectAction()]?.action_effect||null}
function effectLayerVisible(kind){if(kind==='prediction')return effectViewMode!=='actual'&&viewSettings.showPrediction;if(kind==='actual')return activeSideTab==='result'&&effectViewMode!=='prediction'&&viewSettings.showActual;return false}
function pointDistance(a,b){return a&&b?Math.hypot(Number(a.x)-Number(b.x),Number(a.y)-Number(b.y)):null}
function drawEffectPoint(position,color,label,{radius=7,dashed=false}={}){if(!position||!Number.isFinite(Number(position.x))||!Number.isFinite(Number(position.y)))return;ctx.save();if(dashed)ctx.setLineDash([4,3]);ctx.beginPath();ctx.arc(Number(position.x),Number(position.y),radius,0,Math.PI*2);ctx.fillStyle=color.replace('1)','0.18)');ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=2.2;ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=color;ctx.font='650 10px "Segoe UI"';ctx.textAlign='left';ctx.fillText(label,Number(position.x)+10,Number(position.y)-9);ctx.restore()}
function drawEffectNormal(position,normal,color){if(!position||!normal)return;const length=48,x=Number(position.x),y=Number(position.y),dx=Number(normal.x)*length,dy=Number(normal.y)*length;ctx.save();ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+dx,y+dy);ctx.stroke();const angle=Math.atan2(dy,dx);ctx.beginPath();ctx.moveTo(x+dx,y+dy);ctx.lineTo(x+dx-8*Math.cos(angle-.45),y+dy-8*Math.sin(angle-.45));ctx.lineTo(x+dx-8*Math.cos(angle+.45),y+dy-8*Math.sin(angle+.45));ctx.closePath();ctx.fill();ctx.restore()}
function drawEffectError(predicted,actual){if(!viewSettings.showErrorLines||!predicted||!actual)return;ctx.save();ctx.setLineDash([5,5]);ctx.strokeStyle='rgba(242,193,78,.9)';ctx.lineWidth=1.6;ctx.beginPath();ctx.moveTo(Number(predicted.x),Number(predicted.y));ctx.lineTo(Number(actual.x),Number(actual.y));ctx.stroke();ctx.restore()}
function drawActionEffectOverlay(){const predicted=activeEffectPrediction(),actual=activeSideTab==='result'?activeEffectActual():null,showPrediction=effectLayerVisible('prediction'),showActual=effectLayerVisible('actual');if(!predicted&&!actual)return;
  const predictedContact=predicted?.first_contact?.primary?.label!=='none'?predicted.first_contact.position:null,actualContact=actual?.first_contact?.valid?actual.first_contact.position:null;if(showPrediction)drawEffectPoint(predictedContact,'rgba(78,168,255,1)','预测接触',{dashed:true});if(showActual)drawEffectPoint(actualContact,'rgba(76,201,132,1)','真实接触');if(showPrediction&&showActual)drawEffectError(predictedContact,actualContact);if(viewSettings.showNormals){if(showPrediction)drawEffectNormal(predictedContact,predicted?.first_contact?.normal,'rgba(78,168,255,.9)');if(showActual)drawEffectNormal(actualContact,actual?.first_contact?.normal,'rgba(76,201,132,.9)')}
  if(viewSettings.showGenerations){for(let index=0;index<3;index++){const prediction=predicted?.generations?.[index],truth=actual?.generations?.[index],predictedPosition=prediction&&Number(prediction.exists_probability)>=.5?prediction.position:null,actualPosition=truth?.exists?truth.position:null;if(showPrediction)drawEffectPoint(predictedPosition,'rgba(181,120,255,1)',`预测新果${index+1}`,{radius:6,dashed:true});if(showActual)drawEffectPoint(actualPosition,'rgba(255,151,76,1)',`真实新果${index+1}`,{radius:6});if(showPrediction&&showActual)drawEffectError(predictedPosition,actualPosition)}}
  if(viewSettings.showQ0Final){const predictedFinal=predicted&&Number(predicted.q0?.final_exists_probability)>=.5?predicted.q0.final:null,actualFinal=actual?.q0?.final_exists?actual.q0.final:null;if(showPrediction)drawEffectPoint(predictedFinal,'rgba(74,205,222,1)','预测 q0 后代',{radius:9,dashed:true});if(showActual)drawEffectPoint(actualFinal,'rgba(255,210,80,1)','真实 q0 后代',{radius:9});if(showPrediction&&showActual)drawEffectError(predictedFinal,actualFinal)}}
function contactLabel(value){return {none:'未接触',floor:'地面',left_wall:'左墙',right_wall:'右墙',fruit:'水果'}[value]||value||'—'}
function contactCandidateLabel(value){if(value==='dynamic_fruit')return '动态水果';if(String(value).startsWith('fruit_slot_'))return String(value).replace('fruit_slot_','水果槽位 ');return contactLabel(value)}
function renderEffectComparison(){const predicted=activeEffectPrediction(),actual=activeEffectActual(),root=$('effect-summary'),detail=$('effect-detail'),action=effectAction();document.querySelectorAll('[data-effect-view]').forEach(button=>button.classList.toggle('active',button.dataset.effectView===effectViewMode));if(!predicted&&!actual){$('effect-state-label').textContent=modelIdentity?.action_effect_available===false?'当前模型没有辅助动作头':'等待模型预测';root.replaceChildren();detail.textContent=modelIdentity?.action_effect_available===false?'当前 checkpoint 未启用 action_effect_enabled，请加载 24M 辅助旁路模型。':'加载带辅助动作头的 checkpoint 后，可以先查看预测位置，再执行真实物理验证。';return}
  $('effect-state-label').textContent=`A${action} · ${predicted&&actual?'已完成对照':predicted?'仅预测':'仅真实'}`;const predictionContact=predicted?.first_contact?.position,actualContact=actual?.first_contact?.position,contactError=predicted&&actual?.first_contact?.valid?pointDistance(predictionContact,actualContact):null,generationPrediction=predicted?.generations?.filter(item=>Number(item.exists_probability)>=.5).length,generationActual=actual?.generations?.filter(item=>item.exists).length;
  const cards=[['首次接触',`${predicted?contactLabel(predicted.first_contact.primary.label):'—'} / ${actual?contactLabel(actual.first_contact.primary):'—'}`],['接触位置误差',contactError===null?'等待真实结果':`${contactError.toFixed(1)} px`],['合成次数',`${predicted?predicted.merge.count.index:'—'} / ${actual?actual.merge.count:'—'}`],['新水果数',`${predicted??false?generationPrediction:'—'} / ${actual?generationActual:'—'}`],['q0 最终等级',`${predicted?predicted.q0.final_level.index:'—'} / ${actual?actual.q0.final_level:'—'}`],['得分增量',`${predicted?Number(predicted.outcome.score_delta).toFixed(1):'—'} / ${actual?actual.outcome.score_delta:'—'}`]];root.replaceChildren(...cards.map(([label,value])=>{const card=document.createElement('div');card.className='effect-card';card.innerHTML='<span></span><strong></strong>';card.querySelector('span').textContent=label;card.querySelector('strong').textContent=value;return card}));
  const lines=['格式：预测 / 真实',`合成概率：${predicted?(100*Number(predicted.merge.probability)).toFixed(1)+'%':'—'} / ${actual?(actual.merge.happened?'是':'否'):'—'}`,`q0参与概率：${predicted?(100*Number(predicted.q0.participated_probability)).toFixed(1)+'%':'—'} / ${actual?(actual.q0.participated?'是':'否'):'—'}`];if(predicted?.first_contact?.target||actual?.first_contact?.target_slot!=null){const predictedTarget=predicted?.first_contact?.target?contactCandidateLabel(predicted.first_contact.target.label):'—',actualTarget=actual?.first_contact?.target_slot==null?'—':`水果槽位 ${actual.first_contact.target_slot}`;lines.push(`结构化接触候选：${predictedTarget} / ${actualTarget}${predicted?.first_contact?.target?' · 置信度 '+(100*Number(predicted.first_contact.target.confidence)).toFixed(1)+'%':''}`)}if(viewSettings.showProbabilities&&predicted){lines.push(`首次接触分布：${predicted.first_contact.primary.probabilities.map((value,index)=>`${contactLabel(['none','floor','left_wall','right_wall','fruit'][index])} ${(100*Number(value)).toFixed(0)}%`).join(' · ')}`);lines.push(`前三次生成概率：${predicted.generations.map(item=>(100*Number(item.exists_probability)).toFixed(0)+'%').join(' / ')}`)}detail.textContent=lines.join('\n')}
function drawSpaceOverlay(){if(!viewSettings.showSpaceEvaluation||activeSideTab!=='result')return;const action=activeResult(),slot=action?.space_slots?.[selectedSpaceSlot];if(!slot)return;const before=slot.before,after=slot.after,xs=after.drop_x||before.drop_x||[];ctx.save();ctx.lineWidth=1;
...
function draw(){ctx.clearRect(0,0,BOARD.width,BOARD.height);const bg=ctx.createLinearGradient(0,0,0,BOARD.height);bg.addColorStop(0,'#17263a');bg.addColorStop(1,'#0e1724');ctx.fillStyle=bg;ctx.fillRect(0,0,BOARD.width,BOARD.height);
  ...
  drawSpaceOverlay();
  const evaluating=evaluationViewActive(),shownAction=evaluating?selectedResultAction:state.probe,shownLevel=evaluating?evaluationScene.queue[0]:state.queue[0],probeX=evaluating?Number(activeResult().drop_x):actionX(shownAction,shownLevel);if(viewSettings.showAnchors){for(let i=0;i<BOARD.actions;i++){const x=actionX(i,shownLevel);ctx.beginPath();ctx.arc(x,BOARD.spawnY,i===shownAction?4:2.2,0,Math.PI*2);ctx.fillStyle=i===shownAction?'#83d2ff':'rgba(189,214,238,.48)';ctx.fill()}ctx.fillStyle='rgba(194,216,237,.58)';ctx.font='9px "Segoe UI"';ctx.textAlign='center';ctx.fillText('A0',actionX(0,shownLevel),BOARD.spawnY+17);ctx.fillText('A10',actionX(10,shownLevel),BOARD.spawnY+17);ctx.fillText('A20',actionX(20,shownLevel),BOARD.spawnY+17)}
  ctx.save();ctx.setLineDash([7,8]);ctx.strokeStyle='rgba(109,203,255,.70)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(probeX,74);ctx.lineTo(probeX,BOARD.height-BOARD.wall);ctx.stroke();ctx.restore();const evaluationFruits=evaluationViewFruits();if(evaluating&&evaluationPhase==='before')drawFruit({level:shownLevel,x:probeX,y:118},{ghost:true});else if(!evaluating&&(!backendConnected||state.livePaused))drawFruit({level:state.queue[0],x:probeX,y:118},{ghost:true});
  const invalid=viewSettings.showWarnings&&!evaluating?overlaps():new Set;(evaluationFruits||visibleSceneFruits()).forEach(fruit=>drawFruit(fruit,{invalid:invalid.has(fruit.id),selected:!evaluationFruits&&fruit.id===state.selectedId}));
  drawActionEffectOverlay();
  if(activeSideTab!=='result')dropPreviews.forEach(preview=>{if(!preview.inside)return;const ghost={level:preview.level,x:preview.x,y:state.livePaused?preview.y:BOARD.spawnY};clampFruit(ghost);drawFruit(ghost,{ghost:true})});
  if(activeSideTab!=='result'&&state.livePaused&&hover&&state.tool==='place'&&!drag&&!hitFruit(hover)&&!dropPreviews.size){const ghost={level:state.level,x:hover.x,y:hover.y};clampFruit(ghost);drawFruit(ghost,{ghost:true})}
}
...
function startPaletteDrag(event,item,button){if(activeSideTab==='result'||modelContinuous.running||autoEditPending||transientEditActive||autoEditFinishing)return;if(event.button!==0&&event.pointerType==='mouse')return;event.preventDefault();state.level=item.level;state.tool='place';const element=document.createElement('img');element.className='fruit-drag-preview';element.src=TEXTURES[item.level];element.alt='';document.body.append(element);button.classList.add('dragging');button.setPointerCapture(event.pointerId);dropPreviews.set(event.pointerId,{level:item.level,x:0,y:0,inside:false,element,button});updateDropPreview(event.pointerId,event.clientX,event.clientY);document.querySelectorAll('.fruit-choice').forEach(choice=>choice.classList.toggle('active',choice===button));document.querySelectorAll('.tool').forEach(tool=>tool.classList.toggle('active',tool.dataset.tool==='place'));renderLiveStatus();draw()}
function renderFruitGrid(){const busy=Boolean(activeSideTab==='result'||modelContinuous.running||autoEditPending||transientEditActive||autoEditFinishing),grid=$('fruit-grid');grid.replaceChildren(...FRUITS.map(item=>{const button=document.createElement('button');button.className='fruit-choice'+(item.level===state.level?' active':'');button.disabled=busy;button.title=modelContinuous.running?'停止模型持续决策后可手工投放':busy?'返回场景设置后可继续投放':`${item.level}级 · ${item.name} · 拖到画布投放`;button.innerHTML=`<img src="${TEXTURES[item.level]}" alt=""><span>${item.name}</span><i>${item.level}</i>`;button.onpointerdown=event=>startPaletteDrag(event,item,button);button.onpointermove=event=>updateDropPreview(event.pointerId,event.clientX,event.clientY);button.onpointerup=event=>finishDropPreview(event.pointerId);button.onpointercancel=event=>finishDropPreview(event.pointerId,true);return button}))}
function renderQueue(){const editable=(!backendConnected||state.livePaused)&&activeSideTab!=='result'&&!modelContinuous.running&&!autoEditPending&&!transientEditActive&&!autoEditFinishing,root=$('queue');root.replaceChildren(...state.queue.map((level,index)=>{const button=document.createElement('button');button.className='queue-slot';button.disabled=!editable;button.title=editable?'点击升级；Shift+点击降级':'返回场景设置并暂停物理后可编辑队列';button.innerHTML=`<em>q${index}</em><img src="${TEXTURES[level]}" alt=""><span>${spec(level).name}</span>`;button.onclick=event=>{if(!editable)return;state.queue[index]=1+(level-1+(event.shiftKey?-1:1)+5)%5;remember();renderUi();draw();syncPausedScene()};return button}))}
function renderSelection(){const fruit=state.fruits.find(item=>item.id===state.selectedId),root=$('selection');if(!fruit){root.className='selection-card empty';root.textContent=modelContinuous.running?'模型正在控制投放，停止后可编辑':backendConnected&&!state.livePaused?'按住场上水果可临时编辑；右键可直接删除':'尚未选择场上水果';return}root.className='selection-card';root.innerHTML=`<img src="${TEXTURES[fruit.level]}" alt=""><div class="selection-copy"><strong>${fruit.level}级 · ${spec(fruit.level).name}</strong><span>x ${fruit.x.toFixed(1)} · y ${fruit.y.toFixed(1)} · 半径 ${spec(fruit.level).radius}</span></div><button class="small-danger" title="删除水果">⌫</button>`;const button=root.querySelector('button');button.disabled=Boolean(modelContinuous.running||autoEditPending||transientEditActive||autoEditFinishing);button.onclick=()=>backendConnected&&!state.livePaused?removeLiveFruit(fruit.id):removeFruit(fruit.id)}
...
function renderLiveStatus(){const autoEditing=Boolean(autoEditPending||transientEditActive||autoEditFinishing),controlled=modelContinuous.running,evaluating=evaluationViewActive(),resultOpen=activeSideTab==='result',predictionReady=Boolean(activeEffectPrediction()),phaseLabel=evaluationPhase==='before'?'投放前':'投放后',label=resultOpen?(evaluating?`评估快照 · ${phaseLabel}`:'评估结果 · 等待快照'):!backendConnected?'离线编辑':state.done?'对局已结束':controlled?`模型持续决策 · #${modelContinuous.decision_count||0}`:autoEditing?'按住编辑 · 松手恢复':state.livePaused?'已暂停 · 编辑模式':'实时运行';$('canvas-status').textContent=evaluating?`${label} · A${selectedResultAction} · ${evaluation.physics_fps||state.fps} FPS`:realtimePredictionInFlight?`${label} · 正在更新预测`:predictionReady?`${label} · 预测 A${effectAction()}`:backendConnected?`${label} · 物理 ${state.physicsFps||120} FPS · 显示同步`:label;$('canvas-tip').textContent=evaluating?`固定显示 A${selectedResultAction} 的${phaseLabel}局面；实时世界仍在后台运行`:resultOpen?'点击“执行并验证”生成真实动作效果快照':predictionReady?'拖动投放探针可直接切换21个动作的预测；场景变化后会自动刷新':controlled?(modelContinuous.message||'模型将在每次连续稳定后自动选择并投放。'):autoEditing?'拖动水果调整位置；松手后自动同步并恢复实时物理':backendConnected&&!state.livePaused?(state.tool==='select'?'选择模式：按住水果直接编辑，右键水果直接删除':'放置模式：空白处拖动投放；按住已有水果直接编辑；右键删除'):'暂停编辑：左键放置 · 拖动调整 · 右键删除 · 滚轮切级';$('live-state-label').textContent=label;$('live-score').textContent=String(state.score||0);$('live-frame').textContent=String(state.physicsFrame||0);$('live-stable').textContent=state.done?'结束':controlled&&modelContinuous.phase==='inference'?'推理中':state.stable?'稳定':'运动中';const toggle=$('live-toggle');toggle.disabled=!backendConnected||state.done||autoEditing;toggle.textContent=state.livePaused?'恢复实时运行':'暂停并进入编辑';toggle.classList.toggle('running',backendConnected&&!state.livePaused&&!state.done);document.querySelector('.canvas-badge i').style.background=resultOpen?'#83d2ff':predictionReady?'#4ea8ff':!backendConnected?'#f2c14e':controlled?'#63c7a0':autoEditing?'#f2c14e':state.livePaused?'#75b9ee':'#79d7a8'}
function renderUi(){const editable=!backendConnected||state.livePaused,busy=Boolean(modelContinuous.running||autoEditPending||transientEditActive||autoEditFinishing),viewLocked=activeSideTab==='result';document.querySelectorAll('.tool').forEach(button=>{button.classList.toggle('active',button.dataset.tool===state.tool);button.disabled=busy||viewLocked||(!editable&&button.dataset.tool==='erase')});renderFruitGrid();renderQueue();renderSelection();renderValidation();renderProbe();
  document.querySelectorAll('[data-fps]').forEach(button=>{button.classList.toggle('active',Number(button.dataset.fps)===state.fps);button.disabled=busy||viewLocked});['template','clear-scene','import','scene-title'].forEach(id=>$(id).disabled=busy||viewLocked);['settle','evaluate','evaluate-top'].forEach(id=>$(id).disabled=busy);canvas.style.cursor=viewLocked||modelContinuous.running?'default':drag?'grabbing':state.tool==='select'?'default':state.tool==='erase'?'not-allowed':'crosshair';applyVisualizationVisibility();renderLiveStatus();renderModelEvaluation();renderEffectComparison();updateUndo()}
...
function selectTab(name){activeSideTab=name==='result'?'result':'scene';document.querySelectorAll('.side-tab').forEach(tab=>tab.classList.toggle('active',tab.dataset.tab===activeSideTab));document.querySelectorAll('.tab-page').forEach(page=>page.classList.toggle('active',page.id===`tab-${activeSideTab}`));renderUi();draw()}
function rewardClass(value){return value>0?'reward-positive':value<0?'reward-negative':''}
function renderModelEvaluation(){const button=$('evaluate-model'),topButton=$('evaluate-model-top'),continuousButton=$('model-continuous'),continuousTop=$('model-continuous-top'),root=$('model-action-results'),running=Boolean(modelContinuous.running),predicting=Boolean(modelBusy||realtimePredictionInFlight);button.disabled=!backendConnected||!modelAvailable||predicting||running;topButton.disabled=!backendConnected||!modelAvailable||predicting||running;button.textContent=predicting?'正在更新预测…':'刷新当前场景预测';topButton.textContent=predicting?'预测更新中…':'刷新模型预测';[continuousButton,continuousTop].forEach(control=>{control.disabled=!backendConnected||!modelContinuousAvailable||modelControlBusy;control.classList.toggle('running',running)});continuousButton.textContent=modelControlBusy?'正在切换…':running?'停止模型持续决策':'启动模型持续决策';continuousTop.textContent=modelControlBusy?'正在切换…':running?'停止持续决策':'启动持续决策';if(!modelEvaluation){$('model-state-label').textContent=running?`持续决策中 · #${modelContinuous.decision_count||0}`:realtimePredictionEnabled()?'实时预测已启用 · 等待稳定场景':modelAvailable?'已加载 · 等待预测':'未加载';$('model-action').textContent='—';$('model-q').textContent='—';$('model-latency').textContent='—';root.replaceChildren();const identity=modelIdentity||{};$('model-note').textContent=running?(modelContinuous.message||'等待第一次模型决策。'):modelAvailable?`${identity.checkpoint||'checkpoint'} · ${identity.checkpoint_sha256||'未知摘要'} · 实时预测不会投放水果。`:'启动服务时通过 --checkpoint 指定模型。';return}const values=modelEvaluation.q_values.map(Number),best=Number(modelEvaluation.action),min=Math.min(...values),max=Math.max(...values),span=Math.max(1e-8,max-min);$('model-state-label').textContent=running?`持续决策中 · #${modelContinuous.decision_count||0}`:realtimePredictionEnabled()?'实时预测 · 已更新':'已完成 · 只读推理';$('model-action').textContent=`A${best} · x ${Number(modelEvaluation.drop_x).toFixed(1)}`;$('model-q').textContent=Number(modelEvaluation.selected_q).toFixed(4);$('model-latency').textContent=`${Number(modelEvaluation.inference_ms).toFixed(2)} ms`;root.replaceChildren(...values.map((value,index)=>{const item=document.createElement('div');item.className='action-result'+(index===best?' best':'')+(index===selectedResultAction?' selected':'');item.title=`A${index} · Q ${value.toFixed(6)}`;item.innerHTML=`<i style="height:${8+82*(value-min)/span}%"></i><span>${index}</span>`;item.onclick=()=>{state.probe=index;selectedResultAction=index;renderProbe();renderModelEvaluation();if(evaluation)renderEvaluation();renderEffectComparison();renderLiveStatus();draw()};return item}));const sameScene=evaluation&&modelEvaluationScene&&evaluationScene&&JSON.stringify(modelEvaluationScene)===JSON.stringify(evaluationScene),modelReward=sameScene?evaluation.actions[best]:null,identity=modelEvaluation.model||modelIdentity||{};$('model-note').textContent=running?`${modelContinuous.message||'持续决策运行中'} · 已投放 ${modelContinuous.decision_count||0} 次。`:modelContinuous.error?modelContinuous.error:modelReward&&viewSettings.showSpaceEvaluation?`模型首选 A${best} 的实际空间奖励 ${Number(modelReward.reward).toFixed(5)}；Reward V2.1 最优为 A${evaluation.best_action}。`:modelReward?`模型首选 A${best} 已完成真实动作效果验证。`:`${identity.checkpoint||'checkpoint'} · ${identity.checkpoint_sha256||'未知摘要'} · Q 值仅表示模型相对偏好。`}
async function evaluateModelScene(scene,{selectModelAction=viewSettings.autoSelectModelAction,quiet=false}={}){const requestId=++modelRequestSerial,response=await fetch('/api/model/evaluate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scene})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);if(!Array.isArray(payload.q_values)||payload.q_values.length!==21)throw new Error('模型结果必须包含21个 Q 值');if(requestId!==modelRequestSerial)return null;modelEvaluation=payload;modelEvaluationScene=JSON.parse(JSON.stringify(scene));modelEvaluationKey=predictionSceneKey(scene);realtimePredictionLastKey=modelEvaluationKey;if(selectModelAction){state.probe=Number(payload.action);selectedResultAction=Number(payload.action)}renderProbe();renderModelEvaluation();renderEffectComparison();renderLiveStatus();draw();if(!quiet)window.dispatchEvent(new CustomEvent('daxigua:model-prediction',{detail:payload}));return payload}
async function requestModelEvaluation(){selectTab('result');if(!backendConnected||!modelAvailable){toast('模型未加载','请使用 --checkpoint 启动场景实验室服务。');return}modelBusy=true;renderModelEvaluation();try{const payload=await evaluateModelScene(cloneScene());toast('模型评估完成',`首选 A${payload.action} · Q ${Number(payload.selected_q).toFixed(4)}`)}catch(error){toast('模型评估失败',String(error.message||error))}finally{modelBusy=false;renderModelEvaluation()}}
async function sendModelControl(command){const response=await fetch('/api/model/control',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);return payload}
async function toggleModelContinuous(){if(!backendConnected||!modelContinuousAvailable||modelControlBusy)return;const stopping=Boolean(modelContinuous.running);modelControlBusy=true;renderModelEvaluation();try{if(!stopping&&state.livePaused){await sendLiveCommand({type:'resume'});state.livePaused=false}modelContinuous=await sendModelControl(stopping?'stop':'start');if(!stopping){evaluation=null;evaluationScene=null;selectedResultAction=null;$('result-empty').hidden=false;$('result-content').hidden=true;selectTab('scene')}else{renderUi()}toast(stopping?'持续决策已停止':'持续决策已启动',modelContinuous.message||'当前局面保持不变。')}catch(error){toast('持续决策切换失败',String(error.message||error))}finally{modelControlBusy=false;renderUi();draw()}}
function renderEvaluation(){if(!evaluation)return;const actions=evaluation.actions.slice(0,21),rewards=actions.map(item=>Number(item.reward)||0),bestIndex=Number.isInteger(evaluation.best_action)?evaluation.best_action:rewards.indexOf(Math.max(...rewards)),best=rewards[bestIndex]||0,current=actions[selectedResultAction]||actions[bestIndex],phaseLabel=evaluationPhase==='before'?'投放前':'投放后';$('best-action').textContent=`A${bestIndex}`;$('best-reward').textContent=best.toFixed(4);$('best-reward').className=rewardClass(best);$('selected-action').textContent=`A${selectedResultAction}`;$('selected-reward').textContent=Number(current.reward).toFixed(4);$('selected-reward').className=rewardClass(Number(current.reward));$('space-before').textContent=Number(current.previous_potential).toFixed(4);$('space-after').textContent=Number(current.next_potential).toFixed(4);$('space-delta').textContent=Number(current.raw_space_delta).toFixed(5);$('space-delta').className=rewardClass(Number(current.raw_space_delta));$('space-compensation').textContent=Number(current.reference_loss??current.compensation).toFixed(5);$('best-score').textContent=String(current.score_delta??'—');$('terminal-actions').textContent=String(actions.filter(item=>item.done).length);$('result-fps').textContent=`${evaluation.physics_fps||state.fps} FPS · Reward V2.1`;$('evaluation-view-label').textContent=`A${selectedResultAction} · ${phaseLabel}`;document.querySelectorAll('[data-evaluation-phase]').forEach(button=>button.classList.toggle('active',button.dataset.evaluationPhase===evaluationPhase));
  const min=Math.min(0,...rewards),max=Math.max(0,...rewards),span=Math.max(1e-8,max-min),root=$('action-results');root.replaceChildren(...Array.from({length:21},(_,index)=>{const value=rewards[index]??0,item=document.createElement('div');item.className='action-result'+(index===bestIndex?' best':'')+(index===selectedResultAction?' selected':'');item.title=`A${index} · Reward V2.1 ${value.toFixed(5)}`;item.innerHTML=`<i style="height:${8+82*(value-min)/span}%;background:${value<0?'linear-gradient(#f08b82,#c42b1c)':''}"></i><span>${index}</span>`;item.onclick=()=>{selectedResultAction=index;renderEvaluation();renderLiveStatus();draw()};return item}));
  const slots=current.space_slots||[];$('space-slots').replaceChildren(...slots.map((slot,index)=>{const button=document.createElement('button'),area=slot.after.effective_normalized_area??slot.after.normalized_area;button.className='space-slot'+(index===selectedSpaceSlot?' active':'');button.innerHTML=`<strong>q${index+1} · ${spec(slot.level).name}</strong>权重 ${Number(slot.weight).toFixed(1)} · 有效面积 ${Number(area).toFixed(3)}${current.done?' · 终局':''}`;button.onclick=()=>{selectedSpaceSlot=index;renderEvaluation();draw()};return button}));$('result-message').textContent=evaluation.message||'结果来自已连接的真实物理与Reward V2.1后端。';renderModelEvaluation();renderEffectComparison();draw()}
async function requestEvaluation(mode='all'){const requestMode=mode==='settle'?'probe':mode,detail={mode:requestMode,scene:cloneScene()};window.dispatchEvent(new CustomEvent('daxigua:scenario-request',{detail}));selectTab('result');if(!backendConnected){toast('后端未连接','请使用 --serve 启动真实物理与Reward V2.1服务。');return}toast('开始真实验证','正在并行执行21个投放动作…');try{if(modelAvailable&&!modelPredictionMatches(detail.scene))await evaluateModelScene(detail.scene);const response=await fetch('/api/evaluate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(detail)}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);applyEvaluation(payload,detail.scene);toast('验证完成',`已返回 ${payload.actions.length} 个真实动作效果${viewSettings.showSpaceEvaluation?'及空间评估结果':''}`)}catch(error){toast('验证失败',String(error.message||error))}}
function applyEvaluation(payload,sourceScene=null){if(!payload||!Array.isArray(payload.actions)||payload.actions.length!==21)throw new Error('评估结果必须包含21个 actions');evaluation=payload;evaluationScene=JSON.parse(JSON.stringify(sourceScene||cloneScene()));evaluationPhase='after';const requestedAction=Number(evaluationScene.probe_action);selectedResultAction=Number.isInteger(requestedAction)&&requestedAction>=0&&requestedAction<21?requestedAction:(Number.isInteger(payload.selected_action)?payload.selected_action:(Number.isInteger(payload.best_action)?payload.best_action:0));state.probe=selectedResultAction;selectedSpaceSlot=0;$('result-empty').hidden=true;$('result-content').hidden=false;renderEvaluation();selectTab('result')}
...
function applyLivePayload(payload){if(!payload||!Array.isArray(payload.fruits)||!Array.isArray(payload.queue))return;if(Number(payload.sequence)<=state.liveSequence)return;const previousRunning=Boolean(modelContinuous.running),previousDecision=Number(modelContinuous.decision_count)||0,previousPhase=modelContinuous.phase,receivedAt=performance.now(),incomingFrame=Number(payload.physics_frame)||0,nextPaused=Boolean(payload.paused),nextFruits=payload.fruits.map(item=>({id:Number(item.id),level:Number(item.level),x:Number(item.x),y:Number(item.y),vx:Number(item.vx)||0,vy:Number(item.vy)||0,angle:Number(item.angle)||0,angularVelocity:Number(item.angular_velocity)||0,ageFrames:Number(item.age_frames)||0,physicsRadius:Number(item.physics_radius)||spec(Number(item.level)).merged_physics_radius}));if(!visualCurrentFrame||nextPaused||incomingFrame<=visualCurrentFrame.physicsFrame){setVisualFruitsExact(nextFruits,incomingFrame)}else{const startFruits=interpolateLiveFruits(receivedAt),frameDelta=Math.max(1,incomingFrame-visualCurrentFrame.physicsFrame),physicsFps=Number(payload.physics_fps)||120;visualPreviousFrame={fruits:startFruits,physicsFrame:visualCurrentFrame.physicsFrame,receivedAt};visualCurrentFrame={fruits:nextFruits,physicsFrame:incomingFrame,receivedAt};visualTransitionMs=Math.max(8,Math.min(50,frameDelta*1000/physicsFps));visualFruits=startFruits;visualFramesReady=true}state.liveSequence=Number(payload.sequence);state.fruits=nextFruits;state.queue=payload.queue.map(Number);state.score=Number(payload.score)||0;state.stepCount=Number(payload.step_count)||0;state.physicsFrame=incomingFrame;state.physicsFps=Number(payload.physics_fps)||120;state.publishFps=Number(payload.publish_fps)||60;state.livePaused=nextPaused;if(!state.livePaused&&state.tool==='erase')state.tool='select';state.stable=Boolean(payload.stable);state.done=Boolean(payload.done);currentPredictionSceneKey=predictionSceneKey(cloneScene());if(payload.model_continuous&&typeof payload.model_continuous==='object'){modelContinuous=payload.model_continuous;const nextDecision=Number(modelContinuous.decision_count)||0;if(modelContinuous.last_evaluation&&nextDecision!==previousDecision){modelEvaluation=modelContinuous.last_evaluation;modelEvaluationScene=null;modelEvaluationKey=currentPredictionSceneKey;state.probe=Number(modelEvaluation.action)}if(Boolean(modelContinuous.running)!==previousRunning){renderUi()}else if(nextDecision!==previousDecision||modelContinuous.phase!==previousPhase){renderModelEvaluation()}}nextId=Math.max(1,...state.fruits.map(item=>item.id+1));if(!state.fruits.some(item=>item.id===state.selectedId))state.selectedId=null;renderLiveStatus();if(state.livePaused||state.done)draw();else ensureLiveRenderLoop();const now=performance.now();if(now-lastDynamicRender>100){lastDynamicRender=now;renderQueue();renderSelection();renderValidation();updateUndo()}scheduleRealtimePrediction({live:true})}
...
canvas.addEventListener('wheel',event=>{event.preventDefault();if(activeSideTab==='result')return;const point=pointFromEvent(event),target=backendConnected&&!state.livePaused?null:(hitFruit(point)||state.fruits.find(item=>item.id===state.selectedId));cycleLevel(event.deltaY>0?1:-1,target)},{passive:false});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('settings-page').classList.contains('open')){event.preventDefault();closeSettings();return}if(/INPUT|SELECT|TEXTAREA/.test(event.target.tagName))return;if(event.key==='Escape'){if(autoEditPending){event.preventDefault();autoEditPending.released=true;autoEditPending.cancelled=true;if(drag)finishTransientEdit(drag,true);return}if(dropPreviews.size){event.preventDefault();[...dropPreviews.keys()].forEach(pointerId=>finishDropPreview(pointerId,true));return}}if(activeSideTab==='result'||modelContinuous.running)return;if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();event.shiftKey?redo():undo();return}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='y'){event.preventDefault();redo();return}if(event.key==='Delete'||event.key==='Backspace'){const selected=state.fruits.find(item=>item.id===state.selectedId);if(selected){event.preventDefault();backendConnected&&!state.livePaused?removeLiveFruit(selected.id):removeFruit(selected.id)}}if(event.key.toLowerCase()==='v')state.tool='select';if(event.key.toLowerCase()==='b')state.tool='place';if(event.key.toLowerCase()==='e'&&(!backendConnected||state.livePaused))state.tool='erase';renderUi();draw()});
document.querySelectorAll('.tool').forEach(button=>button.onclick=()=>{if(modelContinuous.running)return;if(backendConnected&&!state.livePaused&&button.dataset.tool==='erase'){toast('擦除需暂停物理','选择和放置可在运行时随时切换。');return}state.tool=button.dataset.tool;renderUi();draw()});document.querySelectorAll('.side-tab').forEach(button=>button.onclick=()=>selectTab(button.dataset.tab));document.querySelectorAll('[data-evaluation-phase]').forEach(button=>button.onclick=()=>{evaluationPhase=button.dataset.evaluationPhase==='before'?'before':'after';renderEvaluation();renderLiveStatus();draw()});
document.querySelectorAll('[data-effect-view]').forEach(button=>button.onclick=()=>{effectViewMode=button.dataset.effectView;renderEffectComparison();draw()});
document.querySelectorAll('[data-view-setting]').forEach(input=>input.onchange=()=>{viewSettings[input.dataset.viewSetting]=input.checked;saveViewSettings();applyVisualizationVisibility();renderEffectComparison();if(input.dataset.viewSetting==='realtimePrediction'||input.dataset.viewSetting==='showPrediction')scheduleRealtimePrediction({immediate:true});draw()});
$('settings-open').onclick=openSettings;$('settings-close').onclick=closeSettings;$('settings-done').onclick=closeSettings;$('settings-reset').onclick=()=>{viewSettings={...DEFAULT_VIEW_SETTINGS};syncSettingsControls();saveViewSettings();applyVisualizationVisibility();renderEffectComparison();scheduleRealtimePrediction({immediate:true});draw()};$('settings-page').onclick=event=>{if(event.target===$('settings-page'))closeSettings()};
document.querySelectorAll('[data-fps]').forEach(button=>button.onclick=()=>{state.fps=Number(button.dataset.fps);remember();renderUi()});
$('template').onchange=event=>loadTemplate(event.target.value);$('probe').oninput=event=>{state.probe=Number(event.target.value);selectedResultAction=state.probe;renderProbe();renderModelEvaluation();renderEffectComparison();draw()};
$('scene-title').onchange=()=>{if(!backendConnected||state.livePaused){remember();syncPausedScene()}};$('undo').onclick=undo;$('redo').onclick=redo;$('clear-scene').onclick=async()=>{if(!backendConnected){loadTemplate('empty');return}try{await sendLiveCommand({type:'clear',queue:[...state.queue]});state.selectedId=null;$('scene-title').value='空白场景';toast('场景已清空','实时物理保持当前运行状态。')}catch(error){toast('清空失败',String(error.message||error))}};$('center-view').onclick=()=>toast('视图已居中','当前画布始终保持完整场地比例。');$('live-toggle').onclick=toggleLive;$('settle').onclick=()=>requestEvaluation('probe');$('evaluate').onclick=()=>requestEvaluation('all');$('evaluate-top').onclick=()=>requestEvaluation('all');$('evaluate-model').onclick=requestModelEvaluation;$('evaluate-model-top').onclick=requestModelEvaluation;$('model-continuous').onclick=toggleModelContinuous;$('model-continuous-top').onclick=toggleModelContinuous;
...
function setBackendStatus(connected,message=''){backendConnected=Boolean(connected);if(!backendConnected){modelAvailable=false;modelContinuousAvailable=false;modelIdentity=null;modelContinuous={running:false,phase:'idle',decision_count:0}}renderUi();if(message)toast(backendConnected?'实时后端已连接':'后端状态',message)}
async function connectBackend(){if(location.protocol==='file:'){setBackendStatus(false);return}try{const response=await fetch('/api/health',{cache:'no-store'}),payload=await response.json();if(!response.ok||!payload.ready||!payload.live_physics)throw new Error(payload.error||'实时物理后端未就绪');backendConnected=true;modelAvailable=Boolean(payload.model_available);modelContinuousAvailable=Boolean(payload.model_continuous_available);modelIdentity=payload.model||null;const stateResponse=await fetch('/api/live/state',{cache:'no-store'}),livePayload=await stateResponse.json();if(!stateResponse.ok)throw new Error(livePayload.error||'无法读取实时世界');applyLivePayload(livePayload);openLiveStream();setBackendStatus(true,`${payload.reward_version||'Reward V2.1'} · ${payload.device||'未知设备'} · 持续物理${modelAvailable?' · 模型已加载':''}${modelContinuousAvailable?' · 持续决策可用':''}`);scheduleRealtimePrediction({immediate:true})}catch(error){setBackendStatus(false,String(error.message||error))}}
window.daxiguaScenarioLab={getScene:cloneScene,loadScene,applyEvaluation,evaluateModel:requestModelEvaluation,toggleContinuous:toggleModelContinuous,setBackendStatus};
