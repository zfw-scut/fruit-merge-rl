...
function invalidateEvaluation(){evaluation=null;evaluationScene=null;selectedResultAction=null;$('result-empty').hidden=false;$('result-content').hidden=true;draw()}
...
function updateUndo(){const editable=(!backendConnected||state.livePaused)&&activeSideTab!=='result'&&!autoEditPending&&!transientEditActive&&!autoEditFinishing;$('undo').disabled=!editable||history.length<2;$('redo').disabled=!editable||!future.length}
...
function startPaletteDrag(event,item,button){if(activeSideTab==='result'||autoEditPending||transientEditActive||autoEditFinishing)return;if(event.button!==0&&event.pointerType==='mouse')return;event.preventDefault();state.level=item.level;state.tool='place';const element=document.createElement('img');element.className='fruit-drag-preview';element.src=TEXTURES[item.level];element.alt='';document.body.append(element);button.classList.add('dragging');button.setPointerCapture(event.pointerId);dropPreviews.set(event.pointerId,{level:item.level,x:0,y:0,inside:false,element,button});updateDropPreview(event.pointerId,event.clientX,event.clientY);document.querySelectorAll('.fruit-choice').forEach(choice=>choice.classList.toggle('active',choice===button));document.querySelectorAll('.tool').forEach(tool=>tool.classList.toggle('active',tool.dataset.tool==='place'));renderLiveStatus();draw()}
function renderFruitGrid(){const busy=Boolean(activeSideTab==='result'||autoEditPending||transientEditActive||autoEditFinishing),grid=$('fruit-grid');grid.replaceChildren(...FRUITS.map(item=>{const button=document.createElement('button');button.className='fruit-choice'+(item.level===state.level?' active':'');button.disabled=busy;button.title=busy?'返回场景设置后可继续投放':`${item.level}级 · ${item.name} · 拖到画布投放`;button.innerHTML=`<img src="${TEXTURES[item.level]}" alt=""><span>${item.name}</span><i>${item.level}</i>`;button.onpointerdown=event=>startPaletteDrag(event,item,button);button.onpointermove=event=>updateDropPreview(event.pointerId,event.clientX,event.clientY);button.onpointerup=event=>finishDropPreview(event.pointerId);button.onpointercancel=event=>finishDropPreview(event.pointerId,true);return button}))}
function renderQueue(){const editable=(!backendConnected||state.livePaused)&&activeSideTab!=='result'&&!autoEditPending&&!transientEditActive&&!autoEditFinishing,root=$('queue');root.replaceChildren(...state.queue.map((level,index)=>{const button=document.createElement('button');button.className='queue-slot';button.disabled=!editable;button.title=editable?'点击升级；Shift+点击降级':'返回场景设置并暂停物理后可编辑队列';button.innerHTML=`<em>q${index}</em><img src="${TEXTURES[level]}" alt=""><span>${spec(level).name}</span>`;button.onclick=event=>{if(!editable)return;state.queue[index]=1+(level-1+(event.shiftKey?-1:1)+5)%5;remember();renderUi();draw();syncPausedScene()};return button}))}
...
function renderLiveStatus(){const autoEditing=Boolean(autoEditPending||transientEditActive||autoEditFinishing),evaluating=evaluationViewActive(),resultOpen=activeSideTab==='result',phaseLabel=evaluationPhase==='before'?'投放前':'投放后',label=resultOpen?(evaluating?`评估快照 · ${phaseLabel}`:'评估结果 · 等待快照'):!backendConnected?'离线编辑':state.done?'对局已结束':autoEditing?'按住编辑 · 松手恢复':state.livePaused?'已暂停 · 编辑模式':'实时运行';$('canvas-status').textContent=evaluating?`${label} · A${selectedResultAction} · ${evaluation.physics_fps||state.fps} FPS`:backendConnected?`${label} · 物理 ${state.physicsFps||120} FPS · 显示同步`:label;$('canvas-tip').textContent=evaluating?`固定显示 A${selectedResultAction} 的${phaseLabel}局面；实时世界仍在后台运行`:resultOpen?'点击“评估 21 个动作”生成静态诊断快照':autoEditing?'拖动水果调整位置；松手后自动同步并恢复实时物理':backendConnected&&!state.livePaused?(state.tool==='select'?'选择模式：按住水果直接编辑，右键水果直接删除':'放置模式：空白处拖动投放；按住已有水果直接编辑；右键删除'):'暂停编辑：左键放置 · 拖动调整 · 右键删除 · 滚轮切级';$('live-state-label').textContent=label;$('live-score').textContent=String(state.score||0);$('live-frame').textContent=String(state.physicsFrame||0);$('live-stable').textContent=state.done?'结束':state.stable?'稳定':'运动中';const toggle=$('live-toggle');toggle.disabled=!backendConnected||state.done||autoEditing;toggle.textContent=state.livePaused?'恢复实时运行':'暂停并进入编辑';toggle.classList.toggle('running',backendConnected&&!state.livePaused&&!state.done);document.querySelector('.canvas-badge i').style.background=resultOpen?'#83d2ff':!backendConnected?'#f2c14e':autoEditing?'#f2c14e':state.livePaused?'#75b9ee':'#79d7a8'}
function renderUi(){const editable=!backendConnected||state.livePaused,busy=Boolean(autoEditPending||transientEditActive||autoEditFinishing),viewLocked=activeSideTab==='result';document.querySelectorAll('.tool').forEach(button=>{button.classList.toggle('active',button.dataset.tool===state.tool);button.disabled=busy||viewLocked||(!editable&&button.dataset.tool==='erase')});renderFruitGrid();renderQueue();renderSelection();renderValidation();renderProbe();
  $('show-anchors').checked=state.showAnchors;$('show-grid').checked=state.showGrid;$('show-warnings').checked=state.showWarnings;document.querySelectorAll('[data-fps]').forEach(button=>{button.classList.toggle('active',Number(button.dataset.fps)===state.fps);button.disabled=busy||viewLocked});['template','clear-scene','import','scene-title'].forEach(id=>$(id).disabled=busy||viewLocked);['settle','evaluate','evaluate-top'].forEach(id=>$(id).disabled=busy);canvas.style.cursor=viewLocked?'default':drag?'grabbing':state.tool==='select'?'default':state.tool==='erase'?'not-allowed':'crosshair';renderLiveStatus();updateUndo()}
