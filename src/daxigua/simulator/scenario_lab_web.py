...
.fruit-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.fruit-choice{position:relative;min-width:0;height:68px;touch-action:none;
...
.fruit-drag-preview{position:fixed;left:0;top:0;z-index:80;width:58px;height:58px;display:none;transform:translate(-50%,-50%);pointer-events:none;filter:drop-shadow(0 8px 10px rgba(0,0,0,.28))}.fruit-drag-preview.visible{display:block}.fruit-drag-preview img{width:100%;height:100%;object-fit:contain}
  ...
  <main class="workspace">
    ...
    <section class="panel stage-panel" aria-label="场景编辑画布">
      ...
      <div class="stage-shell">
        <div class="board-frame">
          ...
          <div id="canvas-tip" class="canvas-tip">按住选择或拿起 · 拖动定位 · 松手后自然下落 · 右键删除</div>
    ...
    <aside class="panel"><div class="panel-scroll">
      ...
      <div id="tab-scene" class="tab-page active">
        ...
        <section class="section">
          <div class="run-stack">
            <button id="settle" class="secondary">整理当前场景至稳定</button>
            <button id="evaluate" class="primary">并行评估 21 个动作</button>
          </div>
          <div class="backend-note"><i></i><span>服务模式下，放置或拖动水果后会先用真实物理自然落稳；动作评估仍并行运行21个投放。</span></div>
        </section>
      </div>
...
<div id="fruit-drag-preview" class="fruit-drag-preview" aria-hidden="true"><img alt=""></div>
...
let nextId=1,hover=null,drag=null,heldPlacement=null,placementSource=null,palettePointerId=null,history=[],future=[],historyLock=false;
let evaluation=null,selectedResultAction=null,selectedSpaceSlot=0,backendConnected=false;
let settleRequestId=0,settleAnimationId=0,settling=false,settlePhase='',animationFruits=null;
...
function physicsSnapshot(){const scene=cloneScene();return JSON.stringify([scene.fps,scene.fruits])}
...
function beginPlacement(point,source='canvas'){if(state.fruits.length>=64){toast('无法继续放置','场景最多包含 64 个水果');return false}heldPlacement={id:nextId,level:state.level,x:point.x,y:point.y,physicsRadius:spec(state.level).dropped_physics_radius};placementSource=source;clampFruit(heldPlacement);draw();return true}
function releasePlacement(){if(!heldPlacement)return false;const fruit=heldPlacement;heldPlacement=null;placementSource=null;delete fruit.inside;nextId=Math.max(nextId,fruit.id+1);state.fruits.push(fruit);state.selectedId=fruit.id;remember();renderUi();draw();requestNaturalSettle();return true}
function canvasPointFromClient(clientX,clientY){const rect=canvas.getBoundingClientRect(),inside=clientX>=rect.left&&clientX<=rect.right&&clientY>=rect.top&&clientY<=rect.bottom;return {inside,point:{x:(clientX-rect.left)*BOARD.width/rect.width,y:(clientY-rect.top)*BOARD.height/rect.height}}}
function moveDragPreview(level,clientX,clientY,visible=true){const preview=$('fruit-drag-preview');preview.classList.toggle('visible',visible);preview.style.left=`${clientX}px`;preview.style.top=`${clientY}px`;preview.querySelector('img').src=TEXTURES[level]}
function beginPalettePlacement(event,item,button){if(event.button!==0)return;if(settling){toast('水果正在下落','请等待本次真实物理轨迹播放完毕。');return}if(state.fruits.length>=64){toast('无法继续放置','场景最多包含 64 个水果');return}event.preventDefault();state.level=item.level;state.tool='place';heldPlacement={id:nextId,level:item.level,x:0,y:0,physicsRadius:item.dropped_physics_radius,inside:false};placementSource='palette';palettePointerId=event.pointerId;button.setPointerCapture(event.pointerId);document.querySelectorAll('.tool').forEach(tool=>tool.classList.toggle('active',tool.dataset.tool==='place'));document.querySelectorAll('.fruit-choice').forEach(choice=>choice.classList.toggle('active',choice===button));moveDragPreview(item.level,event.clientX,event.clientY,true);draw()}
function updatePalettePlacement(event){if(placementSource!=='palette'||event.pointerId!==palettePointerId||!heldPlacement)return;const location=canvasPointFromClient(event.clientX,event.clientY);heldPlacement.inside=location.inside;if(location.inside){heldPlacement.x=location.point.x;heldPlacement.y=location.point.y;clampFruit(heldPlacement)}moveDragPreview(heldPlacement.level,event.clientX,event.clientY,!location.inside);draw()}
function finishPalettePlacement(event,cancelled=false){if(placementSource!=='palette'||event.pointerId!==palettePointerId)return;const shouldRelease=!cancelled&&Boolean(heldPlacement?.inside);moveDragPreview(heldPlacement?.level||state.level,event.clientX,event.clientY,false);palettePointerId=null;if(shouldRelease)releasePlacement();else{heldPlacement=null;placementSource=null;draw()}}
...
function draw(){ctx.clearRect(0,0,BOARD.width,BOARD.height);const bg=ctx.createLinearGradient(0,0,0,BOARD.height);bg.addColorStop(0,'#17263a');bg.addColorStop(1,'#0e1724');ctx.fillStyle=bg;ctx.fillRect(0,0,BOARD.width,BOARD.height);
  ...
  const invalid=state.showWarnings&&!activeResult()&&!animationFruits?overlaps():new Set,visibleFruits=animationFruits||activeResult()?.result_fruits||state.fruits;visibleFruits.forEach(fruit=>drawFruit(fruit,{invalid:invalid.has(fruit.id),selected:!activeResult()&&fruit.id===state.selectedId}));
  if(heldPlacement&&heldPlacement.inside!==false)drawFruit(heldPlacement,{selected:true});else if(hover&&state.tool==='place'&&!drag&&!hitFruit(hover)){const ghost={level:state.level,x:hover.x,y:hover.y};clampFruit(ghost);drawFruit(ghost,{ghost:true})}
}

function renderFruitGrid(){const grid=$('fruit-grid');grid.replaceChildren(...FRUITS.map(item=>{const button=document.createElement('button');button.className='fruit-choice'+(item.level===state.level?' active':'');button.title=`按住拖入画布 · ${item.level}级 · ${item.name} · 半径 ${item.radius}`;button.innerHTML=`<img src="${TEXTURES[item.level]}" alt=""><span>${item.name}</span><i>${item.level}</i>`;button.addEventListener('pointerdown',event=>beginPalettePlacement(event,item,button));button.onclick=()=>{state.level=item.level;state.tool='place';renderUi();draw()};return button}))}
...
function normalizedFruit(item){return {id:Number(item.id),level:Number(item.level),x:Number(item.x),y:Number(item.y),physicsRadius:Number(item.physics_radius??item.physicsRadius)}}
function interpolateTraceFruits(left,right,amount){const nextById=new Map(right.fruits.map(item=>[Number(item.id),item]));return left.fruits.map(item=>{const next=nextById.get(Number(item.id));if(!next)return normalizedFruit(item);const fruit=normalizedFruit(item);fruit.x=Number(item.x)+(Number(next.x)-Number(item.x))*amount;fruit.y=Number(item.y)+(Number(next.y)-Number(item.y))*amount;fruit.physicsRadius=Number(item.physics_radius)+(Number(next.physics_radius)-Number(item.physics_radius))*amount;return fruit})}
function playSettleTrace(frames,physicsFps,requestId){if(!Array.isArray(frames)||frames.length<2)return Promise.resolve();const animationId=++settleAnimationId,startFrame=Number(frames[0].frame)||0,endFrame=Number(frames.at(-1).frame)||startFrame,fps=Math.max(1,Number(physicsFps)||30),durationFrames=Math.max(1,endFrame-startFrame);let cursor=0;return new Promise(resolve=>{const started=performance.now();function tick(){if(requestId!==settleRequestId||animationId!==settleAnimationId){resolve();return}const target=Math.min(endFrame,startFrame+(performance.now()-started)*fps/1000);while(cursor+1<frames.length&&Number(frames[cursor+1].frame)<=target)cursor++;const left=frames[cursor],right=frames[Math.min(cursor+1,frames.length-1)],span=Math.max(1,Number(right.frame)-Number(left.frame)),amount=Math.max(0,Math.min(1,(target-Number(left.frame))/span));animationFruits=interpolateTraceFruits(left,right,amount);$('canvas-status').textContent=`正在播放真实物理轨迹… ${Math.round(100*(target-startFrame)/durationFrames)}% · ${state.fps} FPS`;draw();if(target>=endFrame){animationFruits=(frames.at(-1).fruits||[]).map(normalizedFruit);draw();resolve();return}requestAnimationFrame(tick)}requestAnimationFrame(tick)})}
async function requestNaturalSettle(options={}){if(!backendConnected)return;const fast=options.fast!==false,requestId=++settleRequestId,source=physicsSnapshot(),selectedId=state.selectedId;settleAnimationId++;animationFruits=null;settling=true;settlePhase='正在计算真实物理轨迹…';renderUi();try{const response=await fetch('/api/settle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scene:cloneScene(),fast})}),payload=await response.json();if(!response.ok)throw new Error(payload.error||`HTTP ${response.status}`);if(requestId!==settleRequestId||source!==physicsSnapshot())return;settlePhase='正在播放真实物理轨迹…';renderUi();await playSettleTrace(payload.trace_frames,payload.physics_fps,requestId);if(requestId!==settleRequestId||source!==physicsSnapshot())return;state.fruits=(payload.fruits||[]).map(normalizedFruit);animationFruits=null;nextId=Math.max(1,...state.fruits.map(item=>item.id+1));state.selectedId=state.fruits.some(item=>item.id===selectedId)?selectedId:null;invalidateEvaluation();if(history.length)history[history.length-1]=snapshot();future=[];if(payload.merge_count)toast('场景已自然落稳',`发生 ${payload.merge_count} 次合成`);else if(payload.done)toast('场景已自然落稳','当前局面触发终局');else if(payload.settle_timeout)toast('物理仍在运动','已到达单次等待上限，可继续编辑或再次整理');}catch(error){if(requestId===settleRequestId)toast('自然落稳失败',String(error.message||error))}finally{if(requestId===settleRequestId){settling=false;settlePhase='';animationFruits=null;renderUi();draw()}}}

canvas.addEventListener('contextmenu',event=>event.preventDefault());
canvas.addEventListener('pointerdown',event=>{if(settling){toast('水果正在下落','请等待本次真实物理轨迹播放完毕。');return}const point=pointFromEvent(event),hit=hitFruit(point);canvas.setPointerCapture(event.pointerId);if(event.button===2||state.tool==='erase'){if(hit)removeFruit(hit.id);return}if(hit){state.selectedId=hit.id;drag={fruit:hit,dx:hit.x-point.x,dy:hit.y-point.y,before:snapshot()};renderUi();draw();return}if(state.tool==='select'){state.selectedId=null;renderUi();draw();return}beginPlacement(point,'canvas')});
canvas.addEventListener('pointermove',event=>{const point=pointFromEvent(event);hover=point;const frame=canvas.parentElement.getBoundingClientRect(),rect=canvas.getBoundingClientRect(),label=$('coordinate');label.style.display='block';label.style.left=`${event.clientX-frame.left+12}px`;label.style.top=`${event.clientY-frame.top+12}px`;label.textContent=`${point.x.toFixed(0)}, ${point.y.toFixed(0)}`;
  if(heldPlacement&&placementSource==='canvas'){heldPlacement.x=point.x;heldPlacement.y=point.y;clampFruit(heldPlacement)}else if(drag){drag.fruit.x=point.x+drag.dx;drag.fruit.y=point.y+drag.dy;clampFruit(drag.fruit);renderSelection();renderValidation()}draw()});
canvas.addEventListener('pointerup',()=>{if(placementSource==='canvas'&&releasePlacement())return;if(drag){const changed=drag.before!==snapshot();if(changed)remember();drag=null;renderUi();draw();if(changed)requestNaturalSettle()}});canvas.addEventListener('pointercancel',()=>{if(heldPlacement&&placementSource==='canvas'){heldPlacement=null;placementSource=null;draw()}if(drag){restore(drag.before);drag=null}});canvas.addEventListener('pointerleave',()=>{hover=null;$('coordinate').style.display='none';if(!drag&&!heldPlacement)draw()});
document.addEventListener('pointermove',updatePalettePlacement);document.addEventListener('pointerup',event=>finishPalettePlacement(event));document.addEventListener('pointercancel',event=>finishPalettePlacement(event,true));
...
$('scene-title').onchange=remember;$('undo').onclick=undo;$('redo').onclick=redo;$('clear-scene').onclick=()=>loadTemplate('empty');$('center-view').onclick=()=>toast('视图已居中','当前画布始终保持完整场地比例。');$('settle').onclick=()=>requestNaturalSettle({fast:false});$('evaluate').onclick=()=>requestEvaluation('all');$('evaluate-top').onclick=()=>requestEvaluation('all');
...
window.daxiguaScenarioLab={getScene:cloneScene,loadScene,applyEvaluation,setBackendStatus,settle:requestNaturalSettle};
