...
let pendingLiveFrame=null,liveRenderScheduled=false,lastDynamicRender=0;
...
function updateUndo(){const editable=!backendConnected||state.livePaused;$('undo').disabled=!editable||history.length<2;$('redo').disabled=!editable||!future.length}
...
function draw(){ctx.clearRect(0,0,BOARD.width,BOARD.height);const bg=ctx.createLinearGradient(0,0,0,BOARD.height);bg.addColorStop(0,'#17263a');bg.addColorStop(1,'#0e1724');ctx.fillStyle=bg;ctx.fillRect(0,0,BOARD.width,BOARD.height);
  ...
  const invalid=state.showWarnings&&!activeResult()?overlaps():new Set;state.fruits.forEach(fruit=>drawFruit(fruit,{invalid:invalid.has(fruit.id),selected:state.livePaused&&fruit.id===state.selectedId}));
...
function startPaletteDrag(event,item,button){if(event.button!==0&&event.pointerType==='mouse')return;event.preventDefault();state.level=item.level;state.tool='place';const element=document.createElement('img');element.className='fruit-drag-preview';element.src=TEXTURES[item.level];element.alt='';document.body.append(element);button.classList.add('dragging');button.setPointerCapture(event.pointerId);dropPreviews.set(event.pointerId,{level:item.level,x:0,y:0,inside:false,element,button});updateDropPreview(event.pointerId,event.clientX,event.clientY);document.querySelectorAll('.fruit-choice').forEach(choice=>choice.classList.toggle('active',choice===button));draw()}
function renderFruitGrid(){const grid=$('fruit-grid');grid.replaceChildren(...FRUITS.map(item=>{const button=document.createElement('button');button.className='fruit-choice'+(item.level===state.level?' active':'');button.title=`${item.level}级 · ${item.name} · 拖到画布投放`;button.innerHTML=`<img src="${TEXTURES[item.level]}" alt=""><span>${item.name}</span><i>${item.level}</i>`;button.onpointerdown=event=>startPaletteDrag(event,item,button);button.onpointermove=event=>updateDropPreview(event.pointerId,event.clientX,event.clientY);button.onpointerup=event=>finishDropPreview(event.pointerId);button.onpointercancel=event=>finishDropPreview(event.pointerId,true);return button}))}
function renderQueue(){const editable=!backendConnected||state.livePaused,root=$('queue');root.replaceChildren(...state.queue.map((level,index)=>{const button=document.createElement('button');button.className='queue-slot';button.disabled=!editable;button.title=editable?'点击升级；Shift+点击降级':'暂停物理后可编辑未来队列';button.innerHTML=`<em>q${index}</em><img src="${TEXTURES[level]}" alt=""><span>${spec(level).name}</span>`;button.onclick=event=>{if(!editable)return;state.queue[index]=1+(level-1+(event.shiftKey?-1:1)+5)%5;remember();renderUi();draw();syncPausedScene()};return button}))}
function renderSelection(){const fruit=state.fruits.find(item=>item.id===state.selectedId),root=$('selection');if(!fruit){root.className='selection-card empty';root.textContent=backendConnected&&!state.livePaused?'实时运行中，暂停后可选择编辑':'尚未选择场上水果';return}root.className='selection-card';root.innerHTML=`<img src="${TEXTURES[fruit.level]}" alt=""><div class="selection-copy"><strong>${fruit.level}级 · ${spec(fruit.level).name}</strong><span>x ${fruit.x.toFixed(1)} · y ${fruit.y.toFixed(1)} · 半径 ${spec(fruit.level).radius}</span></div><button class="small-danger" title="删除水果">⌫</button>`;const button=root.querySelector('button');button.disabled=backendConnected&&!state.livePaused;button.onclick=()=>removeFruit(fruit.id)}
...
function renderLiveStatus(){const label=!backendConnected?'离线编辑':state.done?'对局已结束':state.livePaused?'已暂停 · 编辑模式':'实时运行';$('canvas-status').textContent=backendConnected?`${label} · ${state.physicsFps||120} FPS · F${state.physicsFrame}`:label;$('canvas-tip').textContent=backendConnected&&!state.livePaused?'从左侧拖入水果，或在画布按下拖动、松手投放':'暂停编辑：左键放置 · 拖动调整 · 右键删除 · 滚轮切级';$('live-state-label').textContent=label;$('live-score').textContent=String(state.score||0);$('live-frame').textContent=String(state.physicsFrame||0);$('live-stable').textContent=state.done?'结束':state.stable?'稳定':'运动中';const toggle=$('live-toggle');toggle.disabled=!backendConnected||state.done;toggle.textContent=state.livePaused?'恢复实时运行':'暂停并进入编辑';toggle.classList.toggle('running',backendConnected&&!state.livePaused&&!state.done);document.querySelector('.canvas-badge i').style.background=!backendConnected?'#f2c14e':state.livePaused?'#75b9ee':'#79d7a8'}
function renderUi(){const editable=!backendConnected||state.livePaused;document.querySelectorAll('.tool').forEach(button=>{button.classList.toggle('active',button.dataset.tool===state.tool);button.disabled=!editable});renderFruitGrid();renderQueue();renderSelection();renderValidation();renderProbe();
  $('show-anchors').checked=state.showAnchors;$('show-grid').checked=state.showGrid;$('show-warnings').checked=state.showWarnings;document.querySelectorAll('[data-fps]').forEach(button=>button.classList.toggle('active',Number(button.dataset.fps)===state.fps));canvas.style.cursor=editable?'crosshair':'grab';renderLiveStatus();updateUndo()}
...
function syncLiveScene(paused=state.livePaused){if(!backendConnected)return Promise.resolve();return sendLiveCommand({type:'load_scene',scene:cloneScene(),paused:Boolean(paused)}).catch(error=>toast('场景同步失败',String(error.message||error)))}
function syncPausedScene(){if(backendConnected&&state.livePaused)return syncLiveScene(true);return Promise.resolve()}
function applyLivePayload(payload){if(!payload||!Array.isArray(payload.fruits)||!Array.isArray(payload.queue))return;if(Number(payload.sequence)<state.liveSequence)return;state.liveSequence=Number(payload.sequence);state.fruits=payload.fruits.map(item=>({id:Number(item.id),level:Number(item.level),x:Number(item.x),y:Number(item.y),vx:Number(item.vx)||0,vy:Number(item.vy)||0,angle:Number(item.angle)||0,angularVelocity:Number(item.angular_velocity)||0,ageFrames:Number(item.age_frames)||0,physicsRadius:Number(item.physics_radius)||spec(Number(item.level)).merged_physics_radius}));state.queue=payload.queue.map(Number);state.score=Number(payload.score)||0;state.stepCount=Number(payload.step_count)||0;state.physicsFrame=Number(payload.physics_frame)||0;state.physicsFps=Number(payload.physics_fps)||120;state.livePaused=Boolean(payload.paused);state.stable=Boolean(payload.stable);state.done=Boolean(payload.done);nextId=Math.max(1,...state.fruits.map(item=>item.id+1));if(!state.fruits.some(item=>item.id===state.selectedId))state.selectedId=null;renderLiveStatus();draw();const now=performance.now();if(now-lastDynamicRender>100){lastDynamicRender=now;renderQueue();renderSelection();renderValidation();updateUndo()}}
function scheduleLivePayload(payload){pendingLiveFrame=payload;if(liveRenderScheduled)return;liveRenderScheduled=true;requestAnimationFrame(()=>{liveRenderScheduled=false;const latest=pendingLiveFrame;pendingLiveFrame=null;applyLivePayload(latest)})}
function openLiveStream(){liveEventSource?.close();liveEventSource=new EventSource('/api/live/events');liveEventSource.onmessage=event=>{try{scheduleLivePayload(JSON.parse(event.data))}catch(error){console.error(error)}};liveEventSource.onerror=()=>{$('live-state-label').textContent='状态流重连中…'}}
async function toggleLive(){if(!backendConnected)return;try{const targetPaused=!state.livePaused;await sendLiveCommand({type:targetPaused?'pause':'resume'});state.livePaused=targetPaused;state.selectedId=null;if(targetPaused){history=[snapshot()];future=[]}renderUi();draw();toast(targetPaused?'物理已暂停':'实时物理已恢复',targetPaused?'现在可以拖动、删除或导入水果。':'世界将持续模拟，可随时连续投放。')}catch(error){toast('状态切换失败',String(error.message||error))}}

canvas.addEventListener('contextmenu',event=>event.preventDefault());
canvas.addEventListener('pointerdown',event=>{const point=pointFromEvent(event);canvas.setPointerCapture(event.pointerId);if(backendConnected&&!state.livePaused){if(event.button!==0&&event.pointerType==='mouse')return;event.preventDefault();dropPreviews.set(event.pointerId,{level:state.level,x:point.x,y:point.y,inside:true});draw();return}const hit=hitFruit(point);if(event.button===2||state.tool==='erase'){if(hit)removeFruit(hit.id);return}if(hit){state.selectedId=hit.id;drag={pointerId:event.pointerId,fruit:hit,dx:hit.x-point.x,dy:hit.y-point.y,before:snapshot()};renderUi();draw();return}if(state.tool==='select'){state.selectedId=null;renderUi();draw();return}place(point)});
canvas.addEventListener('pointermove',event=>{const point=pointFromEvent(event);hover=point;const frame=canvas.parentElement.getBoundingClientRect(),rect=canvas.getBoundingClientRect(),label=$('coordinate');label.style.display='block';label.style.left=`${event.clientX-frame.left+12}px`;label.style.top=`${event.clientY-frame.top+12}px`;label.textContent=`${point.x.toFixed(0)}, ${point.y.toFixed(0)}`;
  if(dropPreviews.has(event.pointerId)){updateDropPreview(event.pointerId,event.clientX,event.clientY);return}if(drag&&drag.pointerId===event.pointerId){drag.fruit.x=point.x+drag.dx;drag.fruit.y=point.y+drag.dy;clampFruit(drag.fruit);renderSelection();renderValidation()}draw()});
canvas.addEventListener('pointerup',event=>{if(dropPreviews.has(event.pointerId)){finishDropPreview(event.pointerId);return}if(drag&&drag.pointerId===event.pointerId){if(drag.before!==snapshot()){remember();syncPausedScene()}drag=null;renderUi();draw()}});canvas.addEventListener('pointercancel',event=>{if(dropPreviews.has(event.pointerId))finishDropPreview(event.pointerId,true);if(drag&&drag.pointerId===event.pointerId){restore(drag.before);drag=null}});canvas.addEventListener('pointerleave',()=>{hover=null;$('coordinate').style.display='none';if(!drag)draw()});
...
document.addEventListener('keydown',event=>{if(/INPUT|SELECT|TEXTAREA/.test(event.target.tagName))return;if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();event.shiftKey?redo():undo();return}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='y'){event.preventDefault();redo();return}if(event.key==='Delete'||event.key==='Backspace'){const selected=state.fruits.find(item=>item.id===state.selectedId);if(selected){event.preventDefault();removeFruit(selected.id)}}if(!backendConnected||state.livePaused){if(event.key.toLowerCase()==='v')state.tool='select';if(event.key.toLowerCase()==='b')state.tool='place';if(event.key.toLowerCase()==='e')state.tool='erase'}renderUi()});
document.querySelectorAll('.tool').forEach(button=>button.onclick=()=>{if(backendConnected&&!state.livePaused)return;state.tool=button.dataset.tool;renderUi();draw()});document.querySelectorAll('.side-tab').forEach(button=>button.onclick=()=>selectTab(button.dataset.tab));
