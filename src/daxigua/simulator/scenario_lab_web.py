...
let nextId=1,hover=null,drag=null,heldPlacement=null,history=[],future=[],historyLock=false;
...
function beginPlacement(point){if(state.fruits.length>=64){toast('无法继续放置','场景最多包含 64 个水果');return false}heldPlacement={id:nextId,level:state.level,x:point.x,y:point.y,physicsRadius:spec(state.level).dropped_physics_radius};clampFruit(heldPlacement);draw();return true}
function releasePlacement(){if(!heldPlacement)return false;const fruit=heldPlacement;heldPlacement=null;nextId=Math.max(nextId,fruit.id+1);state.fruits.push(fruit);state.selectedId=fruit.id;remember();renderUi();draw();requestNaturalSettle();return true}
...
function draw(){ctx.clearRect(0,0,BOARD.width,BOARD.height);const bg=ctx.createLinearGradient(0,0,0,BOARD.height);bg.addColorStop(0,'#17263a');bg.addColorStop(1,'#0e1724');ctx.fillStyle=bg;ctx.fillRect(0,0,BOARD.width,BOARD.height);
  ...
  if(heldPlacement)drawFruit(heldPlacement,{selected:true});else if(hover&&state.tool==='place'&&!drag&&!hitFruit(hover)){const ghost={level:state.level,x:hover.x,y:hover.y};clampFruit(ghost);drawFruit(ghost,{ghost:true})}
}
...
canvas.addEventListener('pointermove',event=>{const point=pointFromEvent(event);hover=point;const frame=canvas.parentElement.getBoundingClientRect(),rect=canvas.getBoundingClientRect(),label=$('coordinate');label.style.display='block';label.style.left=`${event.clientX-frame.left+12}px`;label.style.top=`${event.clientY-frame.top+12}px`;label.textContent=`${point.x.toFixed(0)}, ${point.y.toFixed(0)}`;
  if(heldPlacement){heldPlacement.x=point.x;heldPlacement.y=point.y;clampFruit(heldPlacement)}else if(drag){drag.fruit.x=point.x+drag.dx;drag.fruit.y=point.y+drag.dy;clampFruit(drag.fruit);renderSelection();renderValidation()}draw()});
canvas.addEventListener('pointerup',()=>{if(releasePlacement())return;if(drag){const changed=drag.before!==snapshot();if(changed)remember();drag=null;renderUi();draw();if(changed)requestNaturalSettle()}});canvas.addEventListener('pointercancel',()=>{if(heldPlacement){heldPlacement=null;draw()}if(drag){restore(drag.before);drag=null}});canvas.addEventListener('pointerleave',()=>{hover=null;$('coordinate').style.display='none';if(!drag&&!heldPlacement)draw()});
