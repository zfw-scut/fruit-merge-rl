...
<div class="app">
  <header class="topbar">
    ...
    <div class="command-group push"><button id="evaluate-model-top" class="command">评估模型倾向</button><button id="model-continuous-top" class="command model-continuous">启动持续决策</button><button id="evaluate-top" class="primary">评估 21 个动作</button></div>
  </header>
...
function invalidateEvaluation(){evaluation=null;evaluationScene=null;selectedResultAction=null;modelEvaluation=null;modelEvaluationScene=null;$('result-empty').hidden=false;$('result-content').hidden=true;renderModelEvaluation();draw()}
...
function applyEvaluation(payload,sourceScene=null){if(!payload||!Array.isArray(payload.actions)||payload.actions.length!==21)throw new Error('评估结果必须包含21个 actions');evaluation=payload;evaluationScene=JSON.parse(JSON.stringify(sourceScene||cloneScene()));evaluationPhase='after';selectedResultAction=Number.isInteger(payload.selected_action)?payload.selected_action:(Number.isInteger(payload.best_action)?payload.best_action:0);selectedSpaceSlot=0;$('result-empty').hidden=true;$('result-content').hidden=false;renderEvaluation();selectTab('result')}
