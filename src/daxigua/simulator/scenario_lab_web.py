...
function renderSelection(){const fruit=state.fruits.find(item=>item.id===state.selectedId),root=$('selection');if(!fruit){root.className='selection-card empty';root.textContent=backendConnected&&!state.livePaused?'使用“选择”工具可查看运动中的水果；修改位置需先暂停':'尚未选择场上水果';return}root.className='selection-card';root.innerHTML=`<img src="${TEXTURES[fruit.level]}" alt=""><div class="selection-copy"><strong>${fruit.level}级 · ${spec(fruit.level).name}</strong><span>x ${fruit.x.toFixed(1)} · y ${fruit.y.toFixed(1)} · 半径 ${spec(fruit.level).radius}</span></div><button class="small-danger" title="删除水果">⌫</button>`;const button=root.querySelector('button');button.disabled=backendConnected&&!state.livePaused;button.onclick=()=>removeFruit(fruit.id)}
...
function renderLiveStatus(){const label=!backendConnected?'离线编辑':state.done?'对局已结束':state.livePaused?'已暂停 · 编辑模式':'实时运行';$('canvas-status').textContent=backendConnected?`${label} · 物理 ${state.physicsFps||120} FPS · 显示同步`:label;$('canvas-tip').textContent=backendConnected&&!state.livePaused?(state.tool==='select'?'选择模式：点击运动中的水果查看状态，修改位置需先暂停':'放置模式：从左侧拖入水果，或在画布按下拖动、松手投放'):'暂停编辑：左键放置 · 拖动调整 · 右键删除 · 滚轮切级';$('live-state-label').textContent=label;$('live-score').textContent=String(state.score||0);$('live-frame').textContent=String(state.physicsFrame||0);$('live-stable').textContent=state.done?'结束':state.stable?'稳定':'运动中';const toggle=$('live-toggle');toggle.disabled=!backendConnected||state.done;toggle.textContent=state.livePaused?'恢复实时运行':'暂停并进入编辑';toggle.classList.toggle('running',backendConnected&&!state.livePaused&&!state.done);document.querySelector('.canvas-badge i').style.background=!backendConnected?'#f2c14e':state.livePaused?'#75b9ee':'#79d7a8'}
function renderUi(){const editable=!backendConnected||state.livePaused;document.querySelectorAll('.tool').forEach(button=>{button.classList.toggle('active',button.dataset.tool===state.tool);button.disabled=!editable&&button.dataset.tool==='erase'});renderFruitGrid();renderQueue();renderSelection();renderValidation();renderProbe();
  $('show-anchors').checked=state.showAnchors;$('show-grid').checked=state.showGrid;$('show-warnings').checked=state.showWarnings;document.querySelectorAll('[data-fps]').forEach(button=>button.classList.toggle('active',Number(button.dataset.fps)===state.fps));canvas.style.cursor=state.tool==='select'?'default':state.tool==='erase'?'not-allowed':'crosshair';renderLiveStatus();updateUndo()}
...
document.addEventListener('keydown',event=>{if(/INPUT|SELECT|TEXTAREA/.test(event.target.tagName))return;if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='z'){event.preventDefault();event.shiftKey?redo():undo();return}if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='y'){event.preventDefault();redo();return}if(event.key==='Delete'||event.key==='Backspace'){const selected=state.fruits.find(item=>item.id===state.selectedId);if(selected){event.preventDefault();removeFruit(selected.id)}}if(event.key.toLowerCase()==='v')state.tool='select';if(event.key.toLowerCase()==='b')state.tool='place';if(event.key.toLowerCase()==='e'&&(!backendConnected||state.livePaused))state.tool='erase';renderUi();draw()});
