...
<main id="__ROOT_ID__">
  ...
  <section class="dxr-panel">
    ...
    <details class="dxr-box">
      ...
      <div class="dxr-help"><kbd>Space</kbd> 播放/暂停；<kbd>←</kbd>/<kbd>→</kbd> 单帧；<kbd>Shift</kbd> + 方向键切换投放；<kbd>M</kbd>/<kbd>Shift+M</kbd> 跳到下/上一次合成；<kbd>Home</kbd>/<kbd>End</kbd> 跳到首尾。智能浏览只压缩观看等待，不改变记录中的物理帧、速度或结果。</div>
    </details>
    <details class="dxr-box">
...
<main><aside class="sidebar"><h1>__TITLE__</h1><p>__DESCRIPTION__</p><div class="controls"><input class="search" type="search" placeholder="筛选环境编号或结束状态"><select class="sort"><option value="original">原抽样顺序</option><option value="drops-desc">投放次数：多到少</option><option value="score-desc">分数：高到低</option><option value="env">环境编号</option></select><button class="previous" type="button">上一局</button><button class="next" type="button">下一局</button></div><div class="counter"></div><div class="list"></div></aside><section class="viewer"><div class="viewerbar"><a class="direct" target="_blank" rel="noopener">单独打开</a></div><iframe title="选中的完整局回放" sandbox="allow-scripts allow-same-origin"></iframe></section></main>
<script>
